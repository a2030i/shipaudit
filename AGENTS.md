...
- **أمان الـwebhook (🟡 انتقال):** الدالتان الرسميتان تتحققان من `X-Voxa-Signature` كـHMAC-SHA256 للنص الخام عبر `HATIF_WEBHOOK_SECRET`. إذا حضر توقيع خاطئ يُرفض بلا رجوع للمفتاح. يبقى `?key=` فقط للطلبات غير الموقعة خلال النقل؛ بعد ضبط السر نفسه في هاتف وSupabase يجب ضبط `HATIF_REQUIRE_SIGNATURE=true` وإزالة المفتاح من رابط هاتف.
