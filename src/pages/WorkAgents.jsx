...
const HEALTH_CHECKS = [
  { key: 'zoho', label: 'حداثة بيانات Zoho Books' },
  { key: 'hatif', label: 'مزامنة مكالمات هاتف' },
  { key: 'platform', label: 'بيانات متاجر المنصة' },
  { key: 'lead_intake', label: 'استقبال عملاء الحملات' },
  { key: 'webhooks', label: 'صندوق Webhooks' },
  { key: 'zoho_runs', label: 'تشغيل مزامنة زوهو' },
  { key: 'cron', label: 'جدولة الخادم' },
  { key: 'vat', label: 'تحديث تقرير ضريبة القيمة المضافة' },
  { key: 'zoho_api', label: 'استهلاك واجهة Zoho Books' },
];
...
function healthDetail(key, item = {}) {
  if (key === 'zoho') return `آخر مزامنة ناجحة: ${healthTime(item.last_sync)}`;
  if (key === 'hatif') return `آخر سحب للمكالمات: ${healthTime(item.last_sync)}`;
  if (key === 'platform') return `آخر لقطة متاجر: ${healthTime(item.last_sync)}`;
  if (key === 'lead_intake') return `الموظفون المستلمون: ${item.configured_recipients || 0} · الطلبات المستلمة: ${item.received_events || 0}`;
  if (key === 'webhooks') return `فاشلة خلال 24 ساعة: ${item.failed_24h || 0} · عالقة: ${item.stuck || 0}`;
  if (key === 'zoho_runs') return `آخر تشغيل: ${item.last_status === 'succeeded' ? 'ناجح' : item.last_status || 'غير مسجل'} · أخطاء 24 ساعة: ${item.failed_24h || 0}`;
  if (key === 'vat') return `آخر تحديث للضريبة: ${healthTime(item.last_refresh)} · الحد ${item.max_age_minutes || 90} دقيقة`;
  if (key === 'zoho_api') return `استخدام اليوم: ${Number(item.calls_today || 0).toLocaleString('en-US')} · حد التنبيه ${Number(item.warning_calls || 0).toLocaleString('en-US')} · تقييد ${item.rate_limited || 0}`;
  return `المهام الأساسية النشطة: ${item.active_required_jobs || 0} من ${item.required_jobs || 0}`;
}
...
export function LegacySystemAgents({ isActive = true }) {
  ...
  const openAgent = agent => {
    ...
    } else if(agent.agent_key==='management_daily_report') { const c=agent.config||{};setForm({enabled:agent.status==='active',hour:c.hour??c.hours?.[0]??9,minute:c.minute??0}); } else if(agent.agent_key==='integration_health'){const c=agent.config||{};setForm({enabled:agent.status==='active',intervalMinutes:c.interval_minutes??360,zohoMinutes:c.zoho_max_age_minutes??90,hatifMinutes:c.hatif_max_age_minutes??20,platformHours:c.platform_max_age_hours??72});} else setForm(null);
  };
      ...
      {selected && <div className="work-agent-dialog-backdrop" role="dialog" aria-modal="true" aria-label="تأسيس وكيل العمل" onClick={() => setSelected(null)}>
        <Card className="work-agent-dialog-card" onClick={e => e.stopPropagation()}>
          ...
          {selected.agent_key === 'overdue_sadad' && form ? <>
            ...
            <div style={{padding:14,borderRadius:12,background:'var(--surface2)',color:'var(--text2)',lineHeight:1.8,fontSize:13}}><b>التقرير:</b> متأخرات العملاء، فواتير زاتكا المعلقة، العملاء الجدد، العملاء بلا مسؤول، المهام المتأخرة، وأخطاء الوكلاء. يُحفظ داخل النظام فقط ولا يرسل لأي جهة خارجية.</div>
            <label className="agent-toggle"><input type="checkbox" checked={form.enabled} onChange={e=>setForm({...form,enabled:e.target.checked})}/><span><b>تشغيل الموجز كل 12 ساعة</b><small>{form.enabled?'سيُنشأ تقرير أول ثم تقرير ثانٍ بعد 12 ساعة':'لن يُنشأ تقرير مجدول'}</small></span></label>
            <div className="agent-form-grid"><label>وقت التقرير الأول<input type="time" value={`${String(form.hour).padStart(2,'0')}:${String(form.minute).padStart(2,'0')}`} onChange={e=>{const[hour,minute]=e.target.value.split(':').map(Number);setForm({...form,hour,minute});}}/></label><label>وقت التقرير الثاني<input value={`${String((Number(form.hour)+12)%24).padStart(2,'0')}:${String(form.minute).padStart(2,'0')}`} disabled/></label></div>
            {preview?.snapshot&&<div style={{padding:14,border:'1px solid var(--accent)',borderRadius:12,background:'var(--accent-dim)',lineHeight:1.9}}><b>معاينة التقرير</b><br/>المتأخرات: {Number(preview.snapshot.overdue_amount||0).toLocaleString('en-US',{maximumFractionDigits:2})} ر.س ({preview.snapshot.overdue_invoices} فاتورة)<br/>زاتكا معلقة: {preview.snapshot.zatca_pending} · عملاء جدد: {preview.snapshot.new_leads_today}<br/>مهام متأخرة: {preview.snapshot.overdue_tasks} · طلبات واردة بلا مسؤول: {preview.snapshot.unassigned_leads}<br/>مخزون فرص باردة: {preview.snapshot.cold_lead_pool||0}</div>}
            {notice&&<div style={{color:notice.includes('تعذر')?'var(--red)':'var(--green)',fontWeight:700}}>{notice}</div>}
            ...
            <div className="agent-form-grid"><label>تكرار الفحص<select value={form.intervalMinutes} onChange={e=>setForm({...form,intervalMinutes:e.target.value})}><option value="15">كل 15 دقيقة</option><option value="30">كل 30 دقيقة</option><option value="60">كل ساعة</option><option value="360">كل 6 ساعات</option></select></label><label>حد تأخر زوهو (دقيقة)<input type="number" min="15" value={form.zohoMinutes} onChange={e=>setForm({...form,zohoMinutes:e.target.value})}/></label><label>حد تأخر هاتف (دقيقة)<input type="number" min="5" value={form.hatifMinutes} onChange={e=>setForm({...form,hatifMinutes:e.target.value})}/></label><label>حد قدم بيانات المنصة (ساعة)<input type="number" min="1" value={form.platformHours} onChange={e=>setForm({...form,platformHours:e.target.value})}/></label></div>
            {preview?.snapshot&&<div className="integration-health-result" style={{padding:14,border:`1px solid ${preview.snapshot.issue_count?'var(--gold)':'var(--green)'}`,borderRadius:12,background:'var(--surface2)'}}><div style={{lineHeight:1.8,marginBottom:10}}><b>{preview.snapshot.issue_count?`${preview.snapshot.issue_count} من ${HEALTH_CHECKS.length} فحوص تحتاج مراجعة`:`الفحوص ${HEALTH_CHECKS.length} سليمة`}</b><div style={{color:'var(--muted)',fontSize:12}}>يعرض كل مصدر وسبب حالته؛ السجل القديم المعالَج لا يُحسب عطلًا.</div></div><div className="integration-health-list">{HEALTH_CHECKS.map(check=>{const item=preview.snapshot[check.key]||{};return <div className="integration-health-row" key={check.key}><span className={item.healthy?'healthy':'attention'}>{item.healthy?'سليم':'يحتاج مراجعة'}</span><div><b>{check.label}</b><small>{healthDetail(check.key,item)}</small></div></div>})}</div></div>}
