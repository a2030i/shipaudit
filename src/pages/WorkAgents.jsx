      ...
      {selected && <div className="work-agent-dialog-backdrop" role="dialog" aria-modal="true" aria-label="تأسيس وكيل العمل" onClick={() => setSelected(null)}>
        <Card className="work-agent-dialog-card" onClick={e => e.stopPropagation()}>
          ...
          {selected.agent_key === 'overdue_sadad' && form ? <>
            ...
            {preview?.snapshot&&<div className="integration-health-result" style={{padding:14,border:`1px solid ${preview.snapshot.issue_count?'var(--gold)':'var(--green)'}`,borderRadius:12,background:'var(--surface2)'}}><div style={{lineHeight:1.8,marginBottom:10}}><b>{preview.snapshot.issue_count?`${preview.snapshot.issue_count} من ${HEALTH_CHECKS.length} فحوص تحتاج مراجعة`:'الفحوص السبعة سليمة'}</b><div style={{color:'var(--muted)',fontSize:12}}>يعرض كل مصدر وسبب حالته؛ السجل القديم المعالَج لا يُحسب عطلًا.</div></div><div className="integration-health-list">{HEALTH_CHECKS.map(check=>{const item=preview.snapshot[check.key]||{};return <div className="integration-health-row" key={check.key}><span className={item.healthy?'healthy':'attention'}>{item.healthy?'سليم':'يحتاج مراجعة'}</span><div><b>{check.label}</b><small>{healthDetail(check.key,item)}</small></div></div>})}</div></div>}
