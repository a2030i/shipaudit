...
import { configureManagementReportAgent, configureOverdueSadadAgent, configureZatcaWorkAgent, loadRecentAgentRuns, loadWorkAgents, previewManagementReportAgent, previewOverdueSadadAgent, previewZatcaWorkAgent, runManagementReportAgent, runOverdueSadadAgent, runZatcaWorkAgent } from '../lib/workAgentService.js';
...
export default function WorkAgents({ isActive = true }) {
  ...
  const openAgent = agent => {
    ...
    } else if(agent.agent_key==='management_daily_report') { const c=agent.config||{};setForm({enabled:agent.status==='active',hour:c.hour??10,minute:c.minute??0}); } else setForm(null);
  };

  const saveAgent = async () => { setSaving(true); setNotice(''); try { if(selected.agent_key==='zatca_nightly') await configureZatcaWorkAgent(form); else if(selected.agent_key==='management_daily_report') await configureManagementReportAgent(form); else await configureOverdueSadadAgent(form); setNotice('تم حفظ الشروط وإعادة الجدولة.'); await load(); } catch(e) { setNotice(e?.message || 'تعذر الحفظ'); } finally { setSaving(false); } };
  const previewAgent = async () => { setSaving(true); setNotice(''); try { setPreview(selected.agent_key==='zatca_nightly' ? await previewZatcaWorkAgent() : selected.agent_key==='management_daily_report'?await previewManagementReportAgent():await previewOverdueSadadAgent()); } catch(e) { setNotice(e?.message || 'تعذرت المعاينة'); } finally { setSaving(false); } };
  const runAgent = async () => { if (!preview) return; const isZatca=selected.agent_key==='zatca_nightly',isManagement=selected.agent_key==='management_daily_report'; const count=isZatca?preview.count:preview.total; if(!isManagement&&!window.confirm(isZatca?`سيتم إرسال ${count} فاتورة إلى زاتكا عبر Zoho الآن. هل تعتمد التنفيذ؟`:`سيتم تجهيز ${count} رسالة بقالب sadad للإرسال الآن. هل تعتمد التشغيل؟`))return;setSaving(true);try{const r=isManagement?await runManagementReportAgent():isZatca?await runZatcaWorkAgent():await runOverdueSadadAgent();setNotice(isManagement?'تم إنشاء تقرير الإدارة وحفظه في سجل التشغيل.':isZatca?`اكتمل التشغيل: أُرسلت ${r.pushed||0}، وتجاوز ${r.skipped||0}، وفشل ${r.failed||0}.`:`تمت جدولة ${r.queued} رسالة، ويعالجها هاتف الآن.`);await load();}catch(e){setNotice(e?.message||'تعذر التشغيل');}finally{setSaving(false);} };
