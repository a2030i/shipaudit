...
const settlementTypes=new Set(['city','town','village','hamlet','administrative','province','locality','municipality']);
const bestPlaceResult=(_name,items)=>Array.isArray(items)?items.find(item=>settlementTypes.has(item.address_type||item.type))||null:null;
const downloadBlob=(content,type,fileName)=>{const blob=content instanceof Blob?content:new Blob([content],{type}),url=URL.createObjectURL(blob),anchor=document.createElement('a');anchor.href=url;anchor.download=fileName;document.body.appendChild(anchor);anchor.click();anchor.remove();setTimeout(()=>URL.revokeObjectURL(url),1500);};
const excelSafeRow=row=>Object.fromEntries(Object.entries(row).map(([key,value])=>[key,value&&typeof value==='object'?JSON.stringify(value):value??'']));
const enrichPlaceResult=(source,item)=>{if(!item)return {...source,hudhud_status:'not_found',data_source:'Hudhud API',hudhud_raw_result:null};const parts=cleanAddress(item.display_name).split('،').map(v=>v.trim()).filter(Boolean);const region=parts.find(v=>/^منطقة\s/.test(v))||null;const regionIndex=region?parts.indexOf(region):-1;const parent=regionIndex>1?parts[regionIndex-1]:null;const exact=normalizePlaceName(item.name)===normalizePlaceName(source.name||source.name_ar);return {...source,hudhud_status:'matched',data_source:'Hudhud API',hudhud_name:item.name||null,hudhud_type:item.address_type||item.type||null,hudhud_type_ar:placeTypeLabel[item.address_type]||placeTypeLabel[item.type]||item.address_type||item.type||null,parent_place:parent,region,country:parts.at(-1)||null,latitude:item.lat??null,longitude:item.lon??null,display_name:item.display_name||null,hudhud_place_id:item.place_id??null,match_confidence:exact?(region?95:85):60,hudhud_derived_from:'display_name',hudhud_raw_result:item};};
...
export default function PublicShortAddress(){
  ...
  const [singlePlaceName,setSinglePlaceName]=useState('');
  const [singlePlaceResult,setSinglePlaceResult]=useState(null);
  const placeRunRef=useRef(0);
  ...
  const uploadPlaces=async event=>{const file=event.target.files?.[0];event.target.value='';if(!file)return;setPlaceMessage('');try{const parsed=JSON.parse(await file.text());if(!Array.isArray(parsed))throw Error('يجب أن يحتوي الملف على قائمة JSON.');const rows=parsed.filter(row=>row&&typeof row==='object'&&(row.name||row.name_ar||row.name_en));if(!rows.length)throw Error('لم نجد أسماء مدن أو قرى داخل الملف.');setPlaceRows(rows);if(rows.every(row=>row.hudhud_status)){setPlaceCursor(rows.length);setPlaceMessage(`تم تحميل ملف منجز يحتوي على ${rows.length.toLocaleString('ar-SA')} سجل. جاري تقسيمه إلى 4 ملفات دون إعادة المعالجة.`);await downloadPlaces(rows);return;}setPlaceCursor(0);setPlaceMessage(`تم تحميل ${rows.length.toLocaleString('ar-SA')} سجل. بدأ التحليل تلقائيًا وستُنزل 4 ملفات عند الاكتمال.`);await processPlaces(rows,0);}catch(e){setPlaceMessage(e.message||'تعذر قراءة الملف.');}};
  const downloadPlaces=async(rows=placeRows)=>{if(!rows.length)return;const completed=rows.filter(row=>row.hudhud_status==='matched'),incomplete=rows.filter(row=>row.hudhud_status!=='matched'),date=new Date().toISOString().slice(0,10);downloadBlob(JSON.stringify(completed,null,2),'application/json;charset=utf-8',`hudhud-completed-${date}.json`);downloadBlob(JSON.stringify(incomplete,null,2),'application/json;charset=utf-8',`hudhud-incomplete-${date}.json`);const XLSX=await import('xlsx'),makeBook=(items,sheetName)=>{const sheet=XLSX.utils.json_to_sheet(items.map(excelSafeRow)),book=XLSX.utils.book_new();XLSX.utils.book_append_sheet(book,sheet,sheetName);return XLSX.write(rtl(book),{type:'array',bookType:'xlsx'});};downloadBlob(makeBook(completed,'تم'),'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',`hudhud-completed-${date}.xlsx`);downloadBlob(makeBook(incomplete,'لم يتم'),'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',`hudhud-incomplete-${date}.xlsx`);setPlaceMessage(`تم تجهيز 4 ملفات: ${completed.length.toLocaleString('ar-SA')} ناجح و${incomplete.length.toLocaleString('ar-SA')} غير مكتمل. إذا طلب المتصفح الإذن فاختر السماح بالتنزيلات المتعددة.`);};
  const processPlaces=async(rows=placeRows,start=placeCursor)=>{if(placeBusy||start>=rows.length)return;const runId=++placeRunRef.current,next=[...rows];setPlaceBusy(true);let wakeLock=null;try{wakeLock=await navigator.wakeLock?.request?.('screen');}catch{/* Wake Lock is optional. */}for(let index=start;index<next.length&&placeRunRef.current===runId;index++){const source=next[index],name=source.name||source.name_ar||source.name_en;setCurrentPlaceName(String(name));setPlaceMessage(`جاري تحليل ${String(name)} — ${index.toLocaleString('ar-SA')} من ${next.length.toLocaleString('ar-SA')}`);try{const response=await fetch(`${supabase.supabaseUrl}/functions/v1/hudhud-short-address`,{method:'POST',headers:{apikey:supabase.supabaseKey,Authorization:`Bearer ${supabase.supabaseKey}`,'Content-Type':'application/json','x-region':'eu-central-1'},body:JSON.stringify({action:'geocode',query:String(name).trim(),website:''})});const payload=await response.json().catch(()=>null);if(!response.ok||!payload?.ok)throw Error(payload?.error||'فشل البحث');const items=Array.isArray(payload.data)?payload.data:payload.data?.results||[];next[index]=enrichPlaceResult(source,bestPlaceResult(name,items));}catch(e){next[index]={...source,hudhud_status:'error',data_source:'Hudhud API',hudhud_raw_result:null,hudhud_error:e.message||'فشل البحث'};}setPlaceRows([...next]);setPlaceCursor(index+1);await new Promise(resolve=>setTimeout(resolve,1150));}try{await wakeLock?.release?.();}catch{/* Ignore Wake Lock release errors. */}if(placeRunRef.current===runId){setCurrentPlaceName('');setPlaceBusy(false);setPlaceMessage(`اكتملت معالجة ${next.length.toLocaleString('ar-SA')} سجل من هدهد. جاري تجهيز 4 ملفات.`);await downloadPlaces(next);}};
  const searchSinglePlace=async event=>{event.preventDefault();const name=singlePlaceName.trim();if(name.length<2||placeBusy)return;setPlaceBusy(true);setSinglePlaceResult(null);setPlaceMessage('');try{const response=await fetch(`${supabase.supabaseUrl}/functions/v1/hudhud-short-address`,{method:'POST',headers:{apikey:supabase.supabaseKey,Authorization:`Bearer ${supabase.supabaseKey}`,'Content-Type':'application/json','x-region':'eu-central-1'},body:JSON.stringify({action:'geocode',query:name,website:''})});const payload=await response.json().catch(()=>null);if(!response.ok||!payload?.ok)throw Error(payload?.error||'تعذر البحث');const items=Array.isArray(payload.data)?payload.data:payload.data?.results||[],matched=enrichPlaceResult({name},bestPlaceResult(name,items));setSinglePlaceResult(matched);setPlaceMessage(matched.hudhud_status==='matched'?'هذه أول مدينة أو قرية أو هجرة أو نطاق إداري أعاده هدهد للاسم المدخل.':'لم يُرجع هدهد مدينة أو قرية أو هجرة مطابقة؛ تم تجاهل الشوارع والطرق والمحلات والمعالم.');}catch(e){setPlaceMessage(e.message||'تعذر البحث عن الموقع.');}finally{setPlaceBusy(false);}};
  ...
  return <main className="address-page" dir="rtl">
    ...
    <section className="address-card">
      ...
      <section className="place-list-section"><div className="place-list-heading"><span><MapPin size={20}/></span><div><h2>معرفة تبعية مدينة أو قرية</h2><p>ابحث عن اسم واحد مباشرة، أو ارفع قائمة JSON لمعالجة عدة أسماء.</p></div></div><form className="single-place-search" onSubmit={searchSinglePlace}><input value={singlePlaceName} onChange={e=>setSinglePlaceName(e.target.value)} placeholder="مثال: الخوار أو خليص"/><button disabled={placeBusy||singlePlaceName.trim().length<2}><Search size={18}/>{placeBusy?'جاري البحث…':'بحث في هدهد'}</button></form>{singlePlaceResult?.hudhud_status==='matched'&&<div className="single-place-result"><strong>{singlePlaceResult.hudhud_name}</strong><div><span>النوع<b>{singlePlaceResult.hudhud_type_ar}</b></span><span>يتبع<b>{singlePlaceResult.parent_place||'لا توجد تبعية أدنى'}</b></span><span>المنطقة<b>{singlePlaceResult.region||'غير متاحة'}</b></span><span>الإحداثيات<b dir="ltr">{singlePlaceResult.latitude}, {singlePlaceResult.longitude}</b></span></div><small>{singlePlaceResult.display_name} · ثقة المطابقة {singlePlaceResult.match_confidence}%</small></div>}<div className="place-list-divider"><span>أو معالجة قائمة</span></div><div className="place-list-actions"><label className="place-upload"><Upload size={18}/> رفع ملف JSON<input type="file" accept="application/json,.json" onChange={uploadPlaces}/></label><button type="button" onClick={()=>processPlaces()} disabled={placeBusy||!placeRows.length||placeCursor>=placeRows.length}>{placeBusy?'يعمل تلقائيًا…':placeCursor?'استئناف التحليل':'بدء التحليل'}</button><button type="button" onClick={()=>downloadPlaces()} disabled={!placeRows.length||placeBusy||placeCursor<placeRows.length}><Download size={18}/> تنزيل 4 ملفات</button></div>{placeMessage&&<p className="place-list-message">{placeMessage}</p>}{placeRows.length>0&&<><div className="place-progress-status"><strong>{Math.round(placeCursor/placeRows.length*100)}%</strong><span>{placeCursor.toLocaleString('ar-SA')} من {placeRows.length.toLocaleString('ar-SA')}</span>{currentPlaceName&&<small>المدينة الحالية: {currentPlaceName}</small>}</div><div className="place-progress"><span style={{width:`${Math.round(placeCursor/placeRows.length*100)}%`}}/></div><div className="place-preview">{placeRows.slice(Math.max(0,placeCursor-5),Math.max(5,placeCursor)).map((row,index)=><div key={row.id||`${row.name}-${index}`}><strong>{row.name||row.name_ar||row.name_en}</strong><span>{row.hudhud_status==='matched'?[row.hudhud_type_ar,row.parent_place,row.region].filter(Boolean).join(' ← '):row.hudhud_status==='error'?'تعذر مؤقتًا':row.hudhud_status==='not_found'?'لم توجد مطابقة':'بانتظار المعالجة'}</span>{row.match_confidence&&<small>ثقة {row.match_confidence}%</small>}</div>)}</div></>}</section>
    </section>
