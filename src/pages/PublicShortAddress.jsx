...
const canonicalPlaceName=value=>normalizePlaceName(value).replace(/^(مدينه|محافظه|قريه|مركز)\s+/,'').trim();
const bestPlaceResult=(name,items)=>{const expected=canonicalPlaceName(name),exact=[...items].filter(item=>canonicalPlaceName(item.name)===expected),useful={city:6,town:5,village:4,hamlet:3,administrative:2,province:1};return exact.sort((a,b)=>(useful[b.type]||0)-(useful[a.type]||0))[0]||null;};
