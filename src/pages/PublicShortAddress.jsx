...
export default function PublicShortAddress(){
  ...
  const [tool,setTool]=useState('reverse'),[query,setQuery]=useState(''),[points,setPoints]=useState([]),[userLocation,setUserLocation]=useState(null),[mapStyle,setMapStyle]=useState(()=>styleUrl(KEY,MAP_ID)),[mapConfigChecked,setMapConfigChecked]=useState(Boolean(KEY)),[result,setResult]=useState(null),[items,setItems]=useState([]),[selected,setSelected]=useState(null),[busy,setBusy]=useState(false),[error,setError]=useState(''),[copied,setCopied]=useState(false);
  ...
  useEffect(()=>{if(mapStyle)return;fetch('/api/hudhud-map-config').then(r=>r.json()).then(d=>setMapStyle(styleUrl(d.publishableKey,d.mapId))).catch(()=>{}).finally(()=>setMapConfigChecked(true));},[mapStyle]);
  ...
  return <main className="hudhud-page" dir="rtl">
    ...
    <section className="hudhud-shell">
      <div className="hudhud-map" ref={mapNode}>{!mapStyle&&<div className="map-setup"><Map size={38}/><strong>{mapConfigChecked?'مفتاح خريطة هدهد غير مضاف':'جاري تهيئة خريطة هدهد'}</strong><span>{mapConfigChecked?'أضف VITE_HUDHUD_PUBLISHABLE_KEY إلى بيئة Production في Vercel.':'نتحقق من إعداد المفتاح العام…'}</span></div>}<div className="map-hint">{tool==='directions'?'اختر نقطة البداية ثم الوجهة':tool==='matrix'?'اختر من نقطتين إلى 6 نقاط':'اضغط على الخريطة لتحديد الموقع'}</div></div>
      <aside className="hudhud-panel">
        ...
        {value&&<article className="result-card">
          <div className="result-card-head"><span><MapPin size={21}/></span><div><small>نتيجة هدهد</small><h3>{titleOf(value)}</h3></div></div>
          <div className="result-actions result-actions-top"><button onClick={copy}>{copied?<Check size={17}/>:<Copy size={17}/>} {copied?'تم النسخ':'نسخ'}</button><button onClick={share}><Share2 size={17}/> مشاركة</button>{point&&<a target="_blank" rel="noreferrer" href={`https://www.google.com/maps?q=${point.lat},${point.lon}`}><ExternalLink size={17}/> فتح بالخريطة</a>}</div>
          {shortcodeOf(value)&&<div className="shortcode"><span>العنوان المختصر</span><strong>{shortcodeOf(value)}</strong></div>}
          <p>{addressOf(value)}</p>
          <dl>{value?.phone&&<><dt>الهاتف</dt><dd>{value.phone}</dd></>}{value?.website&&<><dt>الموقع</dt><dd>{value.website}</dd></>}{value?.rating&&<><dt>التقييم</dt><dd>{value.rating}</dd></>}</dl>
          <div className="result-actions result-actions-bottom"><button onClick={copy}>{copied?<Check size={17}/>:<Copy size={17}/>} {copied?'تم النسخ':'نسخ'}</button><button onClick={share}><Share2 size={17}/> مشاركة</button>{point&&<a target="_blank" rel="noreferrer" href={`https://www.google.com/maps?q=${point.lat},${point.lon}`}><ExternalLink size={17}/> فتح بالخريطة</a>}</div>
        </article>}
