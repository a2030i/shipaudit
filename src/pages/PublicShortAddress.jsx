...
export default function PublicShortAddress(){
  ...
  useEffect(()=>{if(mapStyle)return;call('map_config').then(d=>setMapStyle(styleUrl(d?.publishableKey,d?.mapId))).finally(()=>setMapConfigChecked(true));},[mapStyle,call]);
  ...
  return <main className="hudhud-page" dir="rtl">
    ...
    <section className="hudhud-shell">
      <div className="hudhud-map" ref={mapNode}>{!mapStyle&&<div className="map-setup"><Map size={38}/><strong>{mapConfigChecked?'مفتاح خريطة هدهد غير متاح':'جاري تهيئة خريطة هدهد'}</strong><span>{mapConfigChecked?'تحقق من VITE_HUDHUD_PUBLISHABLE_KEY في Supabase Secrets.':'نتحقق من إعداد المفتاح العام…'}</span></div>}<div className="map-hint">{tool==='directions'?'اختر نقطة البداية ثم الوجهة':tool==='matrix'?'اختر من نقطتين إلى 6 نقاط':'اضغط على الخريطة لتحديد الموقع'}</div></div>
