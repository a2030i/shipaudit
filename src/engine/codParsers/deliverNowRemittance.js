...
//   • Rows 4..N: data — AWB pattern 'DNL' + digits
//   • Last rows: VAT / totals (no AWB)
//
// NOTE: The 'كشف تحصيلات لمحه' weekly report with 'Barcode' + 'Collection
// Amount' columns and DLX-prefixed AWBs is a DELEX file, not DeliverNow.
// See `delexRemittance.js`. Don't add Barcode / Collection Amount keys
// here — they'll false-match on Delex files.
  ...
  parse(allRows) {
    ...
    if (headerIdx < 0) {
      throw new Error(
        ...
        'إن كان الملف فيه "Barcode" + "Collection Amount" فهو ملف ' +
        'ديلكس Delex — اختر "ديلكس Delex" من القائمة.',
      );
    }
