...
// ─── Email AI assistant (analyze / reply / compose) ───────────────────────────
export async function aiEmailAssist(action, { subject, body, fromName, fromEmail, hint }, signal) {
  let prompt;
  const header = `من: ${fromName || fromEmail}\nالموضوع: ${subject}\n\nالمحتوى:\n${(body || '').slice(0, 3000)}`;

  if (action === 'analyze') {
    prompt = `لخّص هذا الإيميل بـ 4 أسطر بالعربية فقط، بدون مقدمات أو شرح إضافي:
• ملخص: جملة واحدة فقط
• المبلغ: (الرقم فقط، أو "لا يوجد")
• الإجراء: جملة واحدة فقط
• الأولوية: عاجل / عالي / عادي / منخفض

${header}`;
  } else if (action === 'reply') {
    prompt = `أنت مساعد إداري. اكتب رداً مهنياً مختصراً بالعربية على هذا الإيميل.
الرد يكون رسمياً، يؤكد الاستلام ويذكر الإجراء أو يطلب ما يلزم.
اكتب الرد فقط بدون أي شرح.${hint ? `\nتعليمات إضافية: ${hint}` : ''}

${header}`;
  } else { // compose
    prompt = `أنت مساعد إداري. اكتب إيميلاً مهنياً بالعربية.
اكتب الإيميل كاملاً (التحية، المحتوى، التوقيع المهني) بدون أي شرح.
${hint ? `تعليمات: ${hint}` : ''}`;
  }

  return await callOR([{ role: 'user', content: prompt }], { maxTokens: 300, signal });
}

// ─── Email financial analysis ──────────────────────────────────────────────────
export async function aiAnalyzeEmail(subject, body, attachmentNames = []) {
  const prompt = `أنت مساعد محاسبي. حلل هذا الإيميل وحدد إذا كان مالياً أم لا.

موضوع الإيميل: ${subject}
المرفقات: ${attachmentNames.length ? attachmentNames.join(', ') : 'لا توجد'}
محتوى الإيميل:
${body.slice(0, 2000)}
...
أجب بـ JSON فقط:
{
  "is_financial": true,
  "confidence": 0.95,
  "summary": "فاتورة شحن من شركة سمسا للفترة مارس 2026",
  "category": "فاتورة شحن",
  "has_excel": true,
  "has_pdf": false,
  "priority": "normal"
...
أنواع الإيميلات المالية: فاتورة، كشف حساب، تقرير مالي، عقد، إيصال دفع، طلب مالي.
priority: "low" | "normal" | "high" | "urgent"
إذا لم يكن مالياً: is_financial = false`;

  const raw = await callOR([{ role: 'user', content: prompt }], { maxTokens: 400 });
  try {
    const obj = JSON.parse(raw.replace(/```json\s*|```/g, '').trim());
    return {
      is_financial: obj.is_financial ?? false,
      confidence:   obj.confidence   ?? 0,
      summary:      obj.summary      ?? '',
      category:     obj.category     ?? '',
      has_excel:    obj.has_excel    ?? false,
      has_pdf:      obj.has_pdf      ?? false,
      priority:     obj.priority     ?? 'normal',
    };
  } catch {
    return { is_financial: false, confidence: 0, summary: '', category: '', has_excel: false, has_pdf: false, priority: 'normal' };
  }
}
