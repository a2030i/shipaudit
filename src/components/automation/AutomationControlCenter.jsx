...
import {
  ...
  Database, Eye, FileText, Filter, GitBranch, History, LockKeyhole, MoreVertical,
...
import {
  loadAutomationCapabilities, loadAutomationRules, loadAutomationRuns, loadAutomationTemplateContracts,
  previewAutomationRule, saveAutomationRule,
} from '../../lib/workAgentService.js';
import { loadWhatsAppConfig } from '../../lib/whatsappService.js';
import {
  AUTOMATION_FIELD_LABELS, normalizeTemplateContract, templateContractIssue, variablesForContract,
} from '../../lib/automationTemplateContracts.js';

const CATEGORY = {
  sales: 'المبيعات', retention: 'الحفاظ على العملاء', collections: 'التحصيل', operations: 'التشغيل', management: 'الإدارة',
};
...
const EVENT = {
  ...
  never_shipped: 'لم ينفذ أي شحنة', financial_suspension_review: 'استحقاق مراجعة الإيقاف المالي',
  scheduled_digest: 'موعد تقرير الإدارة',
  manual: 'تشغيل يدوي',
};
const AUDIENCE = { customer: 'العميل', employee: 'الموظف المسؤول', management: 'الإدارة', internal_queue: 'قائمة عمل داخلية' };
const ACTION = {
  send_hatif_template: 'إرسال قالب هاتف للعميل', notify_employee_template: 'تنبيه الموظف عبر هاتف',
  create_task: 'إنشاء مهمة للموظف', review_queue: 'إضافة إلى قائمة مراجعة',
  in_app_report: 'تقرير داخل النظام', linked_system_agent: 'وكيل نظام مرتبط', account_action: 'تغيير حالة الحساب',
};
const RISK = { low: 'منخفضة', medium: 'متوسطة', high: 'مرتفعة', critical: 'حرجة' };
const APPROVAL = { none: 'لا يحتاج اعتمادًا', operator_review: 'مراجعة المشغّل', manager_approval: 'اعتماد المدير', explicit_each_run: 'اعتماد صريح لكل تشغيل' };
const STATUS = {
...
const STEPS = [
  ...
  ['template', 'الإجراء والمخرجات', 'ماذا سينفذ ولمن'],
...
const EMPTY_RULE = {
  ...
  audience_type: 'customer', action_type: 'send_hatif_template', action_config: { provider: 'hatif', channel: 'whatsapp', externalWrite: true },
  risk_level: 'medium', approval_policy: 'operator_review',
...
const usesTemplate = rule => ['send_hatif_template', 'notify_employee_template'].includes(rule.action_type || 'send_hatif_template');
const isConsequential = rule => rule.action_type === 'account_action' || rule.risk_level === 'critical' || rule.rule_key === 'financial_suspension_review_31d';
...
function RuleRow({ rule, onOpen }) {
  return <button type="button" className="automation-rule-row" onClick={() => onOpen(rule)}>
    ...
    <span><b>{ACTION[rule.action_type] || rule.action_type || 'إجراء غير محدد'}</b><small>{usesTemplate(rule) ? <span dir="ltr">{rule.template_name || 'قالب غير محدد'}</span> : MODE[rule.execution_mode]}</small></span>
...
function VariableEditor({ variables, onChange, contract, minimum = 1, maximum = Number.POSITIVE_INFINITY }) {
  const normalizedContract = normalizeTemplateContract(contract);
  if (normalizedContract) return <div className="automation-vars">
    {normalizedContract.variable_contract.length ? normalizedContract.variable_contract.map(item => {
      const variable = variables.find(value => Number(value.position) === Number(item.position)) || item;
      return <div className={`automation-var ${item.mode === 'field' ? 'is-field' : ''}`} key={item.position}>
        <code>{`{{${item.position}}}`}</code>
        {item.mode === 'field'
          ? <div className="automation-variable-source"><strong>{item.label}</strong><small>{AUTOMATION_FIELD_LABELS[item.source] || item.source}</small></div>
          : <input value={variable.value || ''} onChange={event => onChange(variables.map(value => Number(value.position) === Number(item.position) ? { ...value, value: event.target.value, mode: 'fixed' } : value))} placeholder={item.label || 'اكتب القيمة الثابتة'}/>}
        <span className="automation-variable-lock"><ShieldCheck size={15}/></span>
      </div>;
    }) : <div className="automation-info"><ShieldCheck size={18}/><span><b>لا توجد متغيرات</b> هذا القالب يرسل نصه المعتمد كما هو.</span></div>}
    <p className="automation-note">مصادر البيانات مقفلة حسب عقد القالب حتى لا يرسل متغير في موضع خاطئ.</p>
  </div>;
...
function AudiencePreview({ preview, loading }) {
  ...
  return <div className="automation-preview">
    ...
    <div className="automation-preview-source"><Database size={15}/><span>المصدر: {preview.source || 'غير محدد'}</span>{preview.freshness?.lamhaAt ? <span>لمحة: {dateTime(preview.freshness.lamhaAt)}</span> : null}{preview.freshness?.zohoAt ? <span>Zoho: {dateTime(preview.freshness.zohoAt)}</span> : null}</div>
...
function RuleDrawer({ rule, templates, canManage, onClose, onSaved }) {
  ...
  const contract = normalizeTemplateContract(templates.find(item => item.template_name === draft.template_name));
  const contractIssue = usesTemplate(draft)
    ? (draft.template_name ? templateContractIssue(contract, draft.template_variables || []) : 'لم يحدد قالب معتمد.')
    : '';
  const consequential = isConsequential(draft);
  const systemLinked = draft.action_type === 'linked_system_agent';
  const patch = value => setDraft(current => ({ ...current, ...value }));
  const current = STEPS[step][0];
  const save = async (statusOverride) => {
    if (systemLinked) { setNotice('هذا الوكيل مرتبط بمشغّل نظام فعلي؛ عدّل جدوله من تبويب «وكلاء النظام».'); return null; }
    ...
    if (usesTemplate(draft) && draft.status === 'active' && draft.execution_mode === 'automatic' && contractIssue) {
      setNotice(contractIssue); setStep(STEPS.findIndex(([key]) => key === 'template')); return null;
    }
    if (consequential && (draft.status === 'active' || draft.execution_mode === 'automatic')) {
      setNotice('قاعدة الإيقاف المالي مقفلة على المراجعة ولا تُفعّل تلقائيًا من مركز الأتمتة.'); setStep(0); return null;
    }
  ...
  const runPreview = async () => {
    if (systemLinked) { setNotice('معاينة هذا الوكيل موجودة في تبويب «وكلاء النظام» حتى تبقى القراءة من المشغّل الفعلي.'); return; }
  ...
  const selectTemplate = templateName => {
    const nextContract = templates.find(item => item.template_name === templateName);
    patch({ template_name: templateName, template_variables: variablesForContract(nextContract, draft.template_variables || []) });
  };
      ...
      <div className="automation-builder">
        ...
        <main>
          {current === 'identity' ? <section>
            ...
            <div className="automation-form-grid"><Field label="الفريق المسؤول"><select value={draft.category} onChange={event => patch({ category: event.target.value })}>{Object.entries(CATEGORY).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field><Field label="طريقة العمل"><select value={draft.execution_mode} onChange={event => patch({ execution_mode: event.target.value })}><option value="preview">{MODE.preview}</option><option value="review">{MODE.review}</option>{!consequential ? <option value="automatic">{MODE.automatic}</option> : null}</select></Field><Field label="حالة القاعدة" hint={consequential ? 'الإجراء الحرج يبقى في المراجعة ويتطلب اعتمادًا صريحًا لكل تشغيل.' : 'نشطة + تنفيذ تلقائي يعني العمل عبر المشغّل المعتمد.'}><select value={draft.status || 'draft'} onChange={event => patch({ status: event.target.value })}><option value="draft">مسودة</option><option value="preview">معاينة</option><option value="review">تحتاج مراجعة</option>{!consequential ? <option value="active">نشطة</option> : null}<option value="paused">متوقفة</option></select></Field><Field label="مستوى المخاطر"><select value={draft.risk_level || 'medium'} onChange={event => patch({ risk_level: event.target.value })}>{Object.entries(RISK).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></Field></div>
          ...
          {current === 'trigger' ? <section>
            ...
            {draft.event_type === 'scheduled_digest' ? <div className="automation-info"><History size={18}/><span><b>موعدان ثابتان بتوقيت السعودية</b>{(draft.trigger_config?.hours || [9, 21]).map(hour => `${String(hour).padStart(2, '0')}:00`).join(' و ')}</span></div> : ['invoice_overdue', 'financial_suspension_review', 'stopped_shipping', 'never_shipped'].includes(draft.event_type) ? <div className="automation-form-grid"><Field label={['invoice_overdue', 'financial_suspension_review'].includes(draft.event_type) ? 'عمر الفاتورة بالأيام' : 'عدد الأيام'}><input type="number" min="1" max="3650" value={draft.trigger_config?.days ?? 5} onChange={event => patch({ trigger_config: { ...draft.trigger_config, days: Number(event.target.value) } })}/></Field>{['invoice_overdue', 'financial_suspension_review'].includes(draft.event_type) ? <Field label="الحد الأدنى التشغيلي"><input type="number" min="0" step="0.01" value={draft.trigger_config?.minAmount ?? 0.5} onChange={event => patch({ trigger_config: { ...draft.trigger_config, minAmount: Number(event.target.value) } })}/><small>يستخدم الفواتير القابلة للتحصيل تشغيليًا، ولا يعد الرصيد الهامشي دينًا تشغيليًا.</small></Field> : <Field label="آلية القياس"><input value="عند عبور الحد مرة واحدة" disabled/></Field>}</div> : <Field label="نافذة التقاط الحدث (ساعة)"><input type="number" min="1" max="720" value={draft.trigger_config?.lookbackHours ?? 24} onChange={event => patch({ trigger_config: { ...draft.trigger_config, lookbackHours: Number(event.target.value) } })}/></Field>}
          ...
          {current === 'audience' ? <section>
            <h3>الجمهور والاستثناءات</h3><p>حدد من يستقبل الإجراء، ثم طبّق هوية الجمهور والاستثناءات قبل أي تنفيذ.</p>
            <Field label="المستلم"><select value={draft.audience_type || 'customer'} onChange={event => patch({ audience_type: event.target.value })}>{Object.entries(AUDIENCE).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></Field>
            <div className="automation-identity-rule"><Users size={18}/><span><b>{draft.audience_type === 'customer' ? (draft.safeguards?.dedupeMode === 'once_per_snapshot_phone' ? 'رسالة واحدة للجوال داخل الفحص نفسه' : 'رسالة واحدة للجوال خلال مدة الحماية') : `المستلم: ${AUDIENCE[draft.audience_type] || draft.audience_type}`}</b><small>{draft.audience_type === 'customer' ? (draft.safeguards?.dedupeMode === 'once_per_snapshot_phone' ? 'متجر جديد في فحص لاحق يستحق ترحيبًا جديدًا، ولو كان جواله مرتبطًا بمتجر سابق.' : 'تُدمج المتاجر المشتركة في رقم واحد خلال مدة الحماية المحددة.') : 'المستلم لا يُستنتج من اسم العميل؛ يجب أن يكون الإسناد أو الدور موثقًا في إعداد الإجراء.'}</small></span></div>
          ...
          {current === 'template' ? <section>
            <h3>الإجراء والمخرجات</h3><p>اختيار الإجراء مستقل عن المحفز؛ لا تتحول المعاينة إلى صلاحية تنفيذ.</p>
            <div className="automation-form-grid"><Field label="نوع الإجراء"><select value={draft.action_type || 'send_hatif_template'} onChange={event => { const action_type = event.target.value; patch({ action_type, template_name: ['send_hatif_template', 'notify_employee_template'].includes(action_type) ? draft.template_name : '', template_variables: ['send_hatif_template', 'notify_employee_template'].includes(action_type) ? draft.template_variables : [] }); }}>{Object.entries(ACTION).map(([value, label]) => <option key={value} value={value} disabled={value === 'account_action'}>{label}</option>)}</select></Field><Field label="سياسة الاعتماد"><select value={draft.approval_policy || 'operator_review'} onChange={event => patch({ approval_policy: event.target.value })}>{Object.entries(APPROVAL).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></Field></div>
            {systemLinked ? <div className="automation-info"><LockKeyhole size={18}/><span><b>مرتبط بمشغّل نظام فعلي</b>الجدولة والمعاينة والتشغيل تُدار من تبويب «وكلاء النظام» حتى لا توجد إعدادات مزدوجة.</span></div> : usesTemplate(draft) ? <>
              <Field label="قالب هاتف"><select value={draft.template_name || ''} onChange={event => selectTemplate(event.target.value)}><option value="">اختر القالب</option>{templates.map(template => <option value={template.template_name} key={template.template_name}>{template.template_name}</option>)}</select></Field>
              <Field label="لغة القالب"><input value="العربية" disabled/></Field>
              {contract ? <div className={`automation-contract ${contractIssue ? 'warning' : 'ready'}`}><span><ShieldCheck size={18}/><b>{contractIssue || 'عقد القالب مكتمل وجاهز للمعاينة'}</b></span><p>{contract.purpose}</p>{contract.notes ? <small>{contract.notes}</small> : null}</div> : draft.template_name ? <div className="automation-warning"><AlertTriangle size={17}/><span>{contractIssue}</span></div> : null}
              <h4 className="automation-subtitle">قيم المتغيرات</h4>
              <VariableEditor variables={draft.template_variables || []} contract={contract} minimum={draft.template_name === 'masrah' ? 2 : 1} maximum={draft.template_name === 'masrah' ? 2 : Number.POSITIVE_INFINITY} onChange={template_variables => patch({ template_variables })}/>
              <div className="automation-template-preview"><small>معاينة الربط</small><strong dir="ltr">{draft.template_name || 'template_name'}</strong>{(draft.template_variables || []).map(variable => <p key={variable.position}><code>{`{{${variable.position}}}`}</code><span>{variable.mode === 'field' ? AUTOMATION_FIELD_LABELS[variable.source] || variable.source : variable.value || 'لم تُكتب القيمة'}</span></p>)}</div>
            </> : <div className="automation-info"><FileText size={18}/><span><b>{ACTION[draft.action_type] || 'إجراء داخلي'}</b>هذا الإجراء لا يستخدم قالب هاتف، وتبقى نتيجته داخل النظام أو قائمة المراجعة.</span></div>}
          </section> : null}
          ...
          {current === 'review' ? <section>
            ...
            <div className="automation-review-summary"><p><span>القاعدة</span><b>{draft.name || 'غير مسماة'}</b></p><p><span>الحالة</span><b>{STATUS[draft.status]?.[0] || draft.status}</b></p><p><span>المحفز</span><b>{EVENT[draft.event_type]}</b></p><p><span>المستلم</span><b>{AUDIENCE[draft.audience_type] || draft.audience_type}</b></p><p><span>الإجراء</span><b>{ACTION[draft.action_type] || draft.action_type}</b></p><p><span>الاعتماد</span><b>{APPROVAL[draft.approval_policy] || draft.approval_policy}</b></p><p><span>طريقة التنفيذ</span><b>{MODE[draft.execution_mode]}</b></p><p><span>المخاطر</span><b>{RISK[draft.risk_level] || draft.risk_level}</b></p><p><span>منع التكرار</span><b>{draft.safeguards?.dedupeMode === 'once_per_snapshot_phone' ? DEDUPE_MODE.once_per_snapshot_phone : `${number(draft.safeguards?.dedupeHours)} ساعة`}</b></p>{draft.rule_key === 'welcome_new_customer' ? <p><span>سياسة الجمعة</span><b>{draft.schedule_config?.deferFridayMorning ? `الصباح مؤجل إلى ${draft.schedule_config?.fridayDeferredUntil || '18:00'}` : 'الإرسال ضمن النافذة المعتادة'}</b></p> : null}</div>
...
function WorkflowMap({ rules, capabilities, onOpen }) {
  const guards = capabilities.filter(item => item.kind === 'guard');
  return <div className="automation-map">
    <div className="automation-map-intro"><GitBranch size={20}/><div><strong>خريطة التشغيل الموحدة</strong><p>كل وكيل يمر بالعقد نفسه. اختلاف الفريق لا يلغي حداثة المصدر أو الاعتماد أو سجل النتيجة.</p></div></div>
    <div className="automation-map-legend" aria-label="مراحل سير الأتمتة">
      {['المحفز', 'الشروط', 'الجمهور', 'الإجراء', 'التحقق', 'السجل'].map((label, index) => <span key={label}><i>{index + 1}</i>{label}</span>)}
    </div>
    <div className="automation-map-rules">
      {rules.map(rule => <button type="button" key={rule.id} onClick={() => onOpen(rule)}>
        <header><span><Status value={rule.status}/><b>{rule.name}</b></span><small className={`risk-${rule.risk_level || 'medium'}`}>مخاطر {RISK[rule.risk_level] || RISK.medium}</small></header>
        <div className="automation-flow">
          <span><small>المحفز</small><b>{EVENT[rule.event_type] || rule.event_type}</b><em>{SOURCE[rule.trigger_source] || rule.trigger_source}</em></span>
          <i>←</i><span><small>القرار</small><b>{(rule.conditions || []).length} شروط</b><em>{(rule.exclusions || []).length} استثناءات</em></span>
          <i>←</i><span><small>المستلم</small><b>{AUDIENCE[rule.audience_type] || rule.audience_type}</b><em>{rule.safeguards?.dedupeMode === 'once_per_snapshot_phone' ? 'هوية اللقطة والجوال' : 'هوية الجوال/السجل'}</em></span>
          <i>←</i><span><small>الإجراء</small><b>{ACTION[rule.action_type] || rule.action_type}</b><em>{APPROVAL[rule.approval_policy] || rule.approval_policy}</em></span>
          <i>←</i><span><small>النتيجة</small><b>{MODE[rule.execution_mode]}</b><em>{rule.last_run_at ? dateTime(rule.last_run_at) : 'لم ينفذ بعد'}</em></span>
        </div>
      </button>)}
    </div>
    <section className="automation-guard-registry"><header><LockKeyhole size={18}/><div><strong>حواجز مشتركة</strong><small>{guards.length} حواجز مسجلة في المحرك</small></div></header><div>{guards.map(guard => <span key={guard.capability_key}><b>{guard.label}</b><small>{guard.description}</small></span>)}</div></section>
  </div>;
}
...
export default function AutomationControlCenter({ LegacyPanel, isActive = true }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [ruleRows, runRows, config, contracts, capabilityRows] = await Promise.all([loadAutomationRules(), loadAutomationRuns(), loadWhatsAppConfig(), loadAutomationTemplateContracts(), loadAutomationCapabilities()]);
      const approvedNames = new Set(config?.templates || []);
      setRules(ruleRows); setRuns(runRows);
      setCapabilities(capabilityRows);
      setTemplates((contracts || []).filter(contract => approvedNames.has(contract.template_name)).map(contract => ({ ...contract, approved: contract.approved && approvedNames.has(contract.template_name) })));
  ...
  return <div className="automation-page">
    ...
    {loading ? <div className="automation-loading"><Spinner/></div> : error ? <Card accent="var(--red)" className="automation-error"><AlertTriangle size={20}/><div><strong>تعذر فتح قواعد الأتمتة</strong><p>{error}</p><Btn size="sm" variant="ghost" onClick={load}>إعادة المحاولة</Btn></div></Card> : <>
      ...
      <nav className="automation-tabs"><button className={tab === 'rules' ? 'active' : ''} onClick={() => setTab('rules')}><SlidersHorizontal size={16}/>القواعد <span>{rules.length}</span></button><button className={tab === 'map' ? 'active' : ''} onClick={() => setTab('map')}><GitBranch size={16}/>خريطة التشغيل</button><button className={tab === 'review' ? 'active' : ''} onClick={() => setTab('review')}><Users size={16}/>بانتظار المراجعة</button><button className={tab === 'runs' ? 'active' : ''} onClick={() => setTab('runs')}><History size={16}/>السجل والنتائج</button><button className={tab === 'system' ? 'active' : ''} onClick={() => setTab('system')}><Settings2 size={16}/>وكلاء النظام</button></nav>
      ...
      {tab === 'map' ? <WorkflowMap rules={rules} capabilities={capabilities} onOpen={setSelected}/> : null}
      {tab === 'review' ? <section className="automation-workspace"><div className="automation-empty"><Users size={28}/><strong>لا توجد دفعات تنفيذ معلقة</strong><span>قواعد المراجعة جاهزة للمعاينة، ولا تتحول إلى إرسال أو إيقاف قبل إنشاء دفعة واعتمادها من المسار المختص.</span></div></section> : null}
    ...
    <style>{`
      ...
      .automation-map{display:grid;gap:14px}.automation-map-intro{display:flex;gap:11px;align-items:flex-start;padding:14px;border:1px solid var(--border);border-radius:13px;background:var(--card)}.automation-map-intro svg{color:var(--accent);flex:none}.automation-map-intro strong{font-size:14px}.automation-map-intro p{margin:4px 0 0;color:var(--muted);font-size:11px;line-height:1.7}.automation-map-legend{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:1px;border:1px solid var(--border);border-radius:12px;overflow:hidden;background:var(--border)}.automation-map-legend span{display:flex;align-items:center;gap:7px;padding:10px;background:var(--surface2);font-size:10.5px;font-weight:800}.automation-map-legend i{width:20px;height:20px;display:grid;place-items:center;border-radius:50%;background:var(--card);color:var(--accent);font-style:normal;font-size:9px}.automation-map-rules{display:grid;gap:8px}.automation-map-rules>button{display:grid;gap:11px;width:100%;padding:13px;border:1px solid var(--border);border-radius:12px;background:var(--card);color:var(--text);font:inherit;text-align:start;cursor:pointer}.automation-map-rules>button:hover,.automation-map-rules>button:focus-visible{border-color:var(--accent);outline:0}.automation-map-rules header{display:flex;align-items:center;justify-content:space-between;gap:10px}.automation-map-rules header>span{display:flex;align-items:center;gap:10px}.automation-map-rules header b{font-size:12.5px}.automation-map-rules header>small{padding:4px 7px;border-radius:999px;background:var(--surface2);color:var(--text2);font-size:9.5px;font-weight:850}.automation-map-rules header>small.risk-critical,.automation-map-rules header>small.risk-high{background:var(--red-soft);color:var(--red)}.automation-flow{display:grid;grid-template-columns:minmax(120px,1fr) 18px minmax(110px,1fr) 18px minmax(110px,1fr) 18px minmax(140px,1.2fr) 18px minmax(110px,1fr);align-items:center;gap:6px}.automation-flow>span{display:grid;gap:2px;min-width:0}.automation-flow>span small{font-size:9px;color:var(--muted)}.automation-flow>span b{font-size:10.5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.automation-flow>span em{font-style:normal;font-size:9px;color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.automation-flow>i{font-style:normal;text-align:center;color:var(--muted)}.automation-guard-registry{border:1px solid var(--border);border-radius:13px;background:var(--card);overflow:hidden}.automation-guard-registry>header{display:flex;gap:9px;align-items:center;padding:12px;border-bottom:1px solid var(--border2)}.automation-guard-registry>header svg{color:var(--green)}.automation-guard-registry>header div{display:grid}.automation-guard-registry>header strong{font-size:12px}.automation-guard-registry>header small{font-size:9.5px;color:var(--muted)}.automation-guard-registry>div{display:grid;grid-template-columns:repeat(3,minmax(0,1fr))}.automation-guard-registry>div>span{display:grid;gap:3px;padding:11px 12px;border-inline-end:1px solid var(--border2);border-bottom:1px solid var(--border2)}.automation-guard-registry b{font-size:10.5px}.automation-guard-registry small{font-size:9px;line-height:1.55;color:var(--muted)}
      ...
      .automation-variable-source{min-height:42px;display:grid;align-content:center;gap:2px;padding:6px 10px;border:1px solid var(--border);border-radius:8px;background:var(--surface2)}.automation-variable-source strong{font-size:11px}.automation-variable-source small{font-size:9.5px;color:var(--muted)}.automation-variable-lock{color:var(--green);display:grid;place-items:center}.automation-contract{display:grid;gap:5px;border:1px solid var(--border);border-inline-start:3px solid var(--green);border-radius:10px;padding:11px 12px;background:var(--green-soft);margin-bottom:14px}.automation-contract.warning{border-inline-start-color:var(--gold);background:var(--gold-soft)}.automation-contract>span{display:flex;align-items:center;gap:7px;color:var(--green)}.automation-contract.warning>span{color:var(--gold-ink)}.automation-contract b{font-size:11px}.automation-contract p{margin:0!important;color:var(--text2)!important;font-size:10.5px!important}.automation-contract small{color:var(--muted);font-size:9.5px;line-height:1.6}
