...
import { estimateLamhaOperationSeconds, runLamhaStoreOperation } from '../lib/lamhaStoreStatusService.js';
...
export default function LamhaFinancialAccountReview({ onClose }) {
  ...
  const stopCandidates = useMemo(() => policyCandidates(rows, results, 'deactivate'), [results, rows]);
  const activateCandidates = useMemo(() => policyCandidates(rows, results, 'activate'), [results, rows]);
  const checked = useMemo(() => [...results.values()].filter(result => result?.ok).length, [results]);
  const errors = useMemo(() => [...results.values()].filter(result => result && !result.ok).length, [results]);
  const filtered = useMemo(() => rows.filter(row => {
    const decision = lamhaFinancialDecision(row, results.get(row.storeId));
  ...
  const runStatusCheck = useCallback(async policyGroup => {
    const targets = rows.filter(row => row.eligible && (!policyGroup || row.policyGroup === policyGroup));
  ...
  const confirmAction = async () => {
    ...
    const invalid = [...selected].filter(storeId => {
      ...
      return !row?.eligible || (review === 'deactivate' ? row.policyGroup !== 'overdue' : row.policyGroup !== 'clear');
    });
    ...
    <div className="lfar" dir="rtl">
      ...
      {source.state === 'available' ? <>
        ...
        <div className="lfar-summary">
          ...
          <button type="button" onClick={() => { setView('activate'); setPage(1); }}><span>مرشحون للتشغيل</span><strong>{COUNT(activateCandidates.length)}</strong><small>لا تجاوز بعد 30 يومًا</small></button>
          <div><span>فُحصت في لمحة</span><strong>{COUNT(checked)}</strong><small>من {COUNT(rows.length)} متجر مرتبط</small></div>
        ...
        <div className="lfar-scan-actions">
          ...
          <Btn variant="accent" icon={<Power size={15}/>} onClick={() => runStatusCheck('clear')} disabled={busy || !rows.some(row => row.eligible && row.policyGroup === 'clear')}>فحص حسابات غير المتجاوزين</Btn>
        ...
        <div className="lfar-list">
          ...
          {pageRows.map(row => {
            ...
            const decision = lamhaFinancialDecision(row, live);
