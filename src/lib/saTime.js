...
export const saDateTime = (d) => {
  if (!d) return '—';
  try { return new Date(d).toLocaleString('ar-SA', { timeZone: TZ, day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }); }
  catch { return String(d).slice(0, 16); }
};
// تاريخ فقط: «٢٣ يوليو ٢٠٢٦»
export const saDate = (d) => {
  if (!d) return '—';
  try { return new Date(d).toLocaleDateString('ar-SA', { timeZone: TZ, year: 'numeric', month: 'short', day: 'numeric' }); }
  catch { return String(d).slice(0, 10); }
};
// تاريخ قصير بلا سنة: «٢٣ يوليو»
export const saDateShort = (d) => {
  if (!d) return '—';
  try { return new Date(d).toLocaleDateString('ar-SA', { timeZone: TZ, day: 'numeric', month: 'short' }); }
  catch { return String(d).slice(0, 10); }
};
// وقت فقط: «٠٧:٢٩»
export const saTime = (d) => {
  if (!d) return '—';
  try { return new Date(d).toLocaleTimeString('ar-SA', { timeZone: TZ, hour: '2-digit', minute: '2-digit' }); }
  catch { return ''; }
};
