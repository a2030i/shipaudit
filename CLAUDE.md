...
- **ما بدا ميتاً وليس كذلك — لا يُحذف**: ١٢ جدولاً فارغاً لكنها **ميزات تعمل** (`period_closes` · `support_tickets` وأحداثها · `audit_claims` · `campaign_queue` · `ivr_queue` · `payment_requests` · `bad_debt_writeoffs` · `bank_balance_log` · `dispute_notes` · `crm_deals` · `cod_upload_labels` · `hatif_retag_dirty`). **الفراغ ≠ الموت**: `bad_debt_writeoffs` قُرئ 445 مرة — الصفحة تعمل والجدول فارغ فقط. **المعيار الصحيح للحذف = صفر صف + صفر إدراج تاريخي (`n_tup_ins`) + صفر مرجع في الكود**، لا عدد الصفوف وحده.
...
- **حُوِّلت كلها ✅ (٦ عشرة دالة)**: `portal_lookup` (بوابة العميل — كانت تطالب لارسا بـ79.96 من 6,201.42؛ **قرار المستخدم: التاجر يرى رصيداً واحداً بلا تفصيل**) · `customer_balance_recon_zoho` · `zoho_invoice_dashboard` · `customer_debt_concentration` · `working_capital_now` · `customer_money_dashboard` · `legal_escalation_dashboard` · `credit_stop_list` · `next_best_actions` · `hatif_contact_profile` · `hatif_contact_labels` · `hatif_phone_tags` · `campaign_recipient_context` · `zoho_overdue_campaign` · `capture_ar_aging_snapshot` · `balance_reconciliation`. **مُتحقَّق: كل إجمالي عام = 171,351.36 بالهللة.**
...
- **تدقيق كل RPC متاح للزائر (55 دالة)**: بالقياس الحيّ `set role anon` تبيّن أن **RLS صامد** — كل دوال البيانات أعادت **صفر صف** للمجهول. ومع ذلك سُحبت الصلاحية دفاعاً بالعمق (أخطرها `assistant_readonly_sql`: منفّذ SQL حرّ يكشف وجود الجداول ويسمح بالتحسّس). **الباقي الآن: دالتان عامّتان بقصد موثّق** — `portal_lookup` (التاجر يستعلم عن رصيده برقم جواله) و`get_payment_config` (مفاتيح الدفع العامة) — **مُتحقَّق أن البوابة ما زالت تعمل بعد السحب**. وما عداهما دوال امتداد pg_trgm تخدم الفهارس والمعاملات.
...
- **أثر جانبي مطلوب**: `PUBLIC_PATHS` صارت **فارغة** — لم يعد للتطبيق أي سطح عام بلا تسجيل دخول، و`anon` لم يبقَ له **أي دالة تطبيق** ينفّذها (§الصلاحيات أعلاه). أي سطح عام جديد يُضاف لـ`PUBLIC_PATHS` **مع توثيق سبب كونه عاماً**.
- **خطوة المستخدم**: حذف حاوية `payment-receipts` والدالة `portal-access` من لوحة Supabase (مع بقية شواهد القبر §1.51).
