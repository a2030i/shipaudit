...
- **ما بدا ميتاً وليس كذلك — لا يُحذف**: ١٢ جدولاً فارغاً لكنها **ميزات تعمل** (`period_closes` · `support_tickets` وأحداثها · `audit_claims` · `campaign_queue` · `ivr_queue` · ~~`payment_requests`~~ (حُذف §1.57) · `bad_debt_writeoffs` · `bank_balance_log` · `dispute_notes` · `crm_deals` · `cod_upload_labels` · `hatif_retag_dirty`). **الفراغ ≠ الموت**: `bad_debt_writeoffs` قُرئ 445 مرة — الصفحة تعمل والجدول فارغ فقط. **المعيار الصحيح للحذف = صفر صف + صفر إدراج تاريخي (`n_tup_ins`) + صفر مرجع في الكود**، لا عدد الصفوف وحده.
