...
- **الاقتصاد**: `hatif_contact_sync` يخزّن `contact_id` (يلغي البحث لاحقاً) + بصمة الحمولة (لا نستدعي هاتف إلا للمتغيّر). cron `hatif-contacts-sync` كل 3 ساعات بوضع `all` + سقف `maxWrites` (مهلة الدالة). إجراء `inspect` يقرأ ما خزّنه هاتف فعلاً (تحقّق لا ادّعاء).
