...
- **🆕 وسم المحادثات عبر v2 (2026-07-22 — قيد التحقق)**: دعم هاتف كشف `POST api.voxa.sa/v2/conversations/service-account/{convo_id}/tags` بجسم `{tagIds:[…]}` — إلصاق وسوم **بالمحادثة** (قيد «لا ربط وسم» يبقى صحيحاً **لجهات الاتصال** v1 فقط). حسب الدعم: `{tagIds:[]}` يمسح كل وسوم المحادثة ⇒ الدلالة المشتبهة **استبدال كامل لا إضافة** — أي مزامنة مستقبلية يجب أن تقرأ الوسوم القائمة وتدمج، ولا تحذف أبداً وسماً خارج الكتالوج (وسوم الفريق). إجراء التجربة `action='convo'` في `hatif-contacts-sync` v3 (يختار آخر عميل استلم حملة من `whatsapp_campaign_sends.conversation_id`، probe قراءة افتراضياً، `write:true` للتجربة الكاملة إلصاق→تبديل→تنظيف، `keep:true` يُبقي الوسم للفحص البصري). النتائج تُوثَّق هنا بعد التشغيل.
...
- **توحيد واتساب (المرحلة 0)**: المكوّن `WaActions` (components/) = اتصال 📞 + **إطلاق حملة ✈️ (الفعل الرئيسي، يدير مودال WhatsAppSendModal بنفسه)** + محادثة حرّة 💬 ثانوية. مزروع في: `PhoneLink` بـCRM (4 مواضع) · التصعيد القانوني · طلبات السداد · درج الكشف الداخلي · Watch (زر إطلاق من صفوف ملف الحملة). **قاعدة: أي هاتف جديد في UI → `WaActions`، لا wa.me مباشرة.**
