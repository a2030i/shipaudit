...
- **⚠️ تصحيح جوهري لقاعدة §1.49 عن الصلاحيات**: «اسحب من `public` ثم امنح صراحةً» **غير كافٍ على Supabase**. `pg_default_acl` في هذا المشروع يحمل `defaclobjtype='f'` بقيمة `{postgres=X, anon=X, authenticated=X, service_role=X}` — أي أن **كل دالة جديدة في `public` تُنشأ بمنح EXECUTE صريح لـ`anon`**، لا موروثاً من PUBLIC، فالسحب من public لا يمسّه (مُتحقَّق: `has_function_privilege('anon',…)` بقيت `true` بعده). **الصحيح: `revoke execute … from public, anon` صراحةً.** أي RPC جديد لا يُقصد به الزائر المجهول يلتزم بها ويُتحقَّق منه بـ`has_function_privilege`.
