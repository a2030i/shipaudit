...
- **القاعدة:** أي صفحة رفع/إدخال جديدة → قسم «الرفع والوارد». لا تُثبّت عناصر جديدة (المثبّت = الرئيسية فقط). كل أيقونة قسم لون مميّز. أي عدسة جديدة لـ«حالة الناقلين» → تبويب في `CarriersWorkspace` لا عنصر nav مسطّح.
