...
- **⚠️ تصحيح جوهري لقاعدة §1.49 عن الصلاحيات** (ثلاث محاولات قبل الإغلاق — كلها مقيسة):
  ...
  - **`ALTER DEFAULT PRIVILEGES` لا يكفي**: سحب anon أعاد منح PUBLIC المدمج؛ وإضافة سحب public جعلت `pg_default_acl` = `{postgres,authenticated,service_role}` **ومع ذلك وُلدت دالة الاختبار بـ`=X`** (مُختبَر مرّتين بدالة تحقّق).
  - **الحلّ الحاسم = event trigger** `trg_lock_new_function_acl` على `ddl_command_end`: يسحب `execute` من `public, anon` عند كل `CREATE/ALTER FUNCTION` في `public`، ويستثني `portal_lookup`/`get_payment_config`. **مُثبَت بالاختبار**: دالة جديدة تولد `anon=false · authenticated=true`. فلا يعتمد الأمان على ذاكرة المطوّر.
  - **القاعدة العملية**: `revoke execute … from public, anon` (الاثنان معاً) — والحارس يتكفّل بالجديد. التحقّق دائماً بـ`has_function_privilege('anon', …)` لا بقراءة الأمر المكتوب.
