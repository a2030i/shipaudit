...
- **الإسناد بالقالب (2026-07-22 — `hatif-webhook v9`)**: قرار المستخدم — المسؤول عن الرد يُحدَّد **بالقالب لا بالمُرسِل** (قالب مالي→بلال، تسويقي→المبيعات). الربط في `whatsapp_config.templateAgents = { [templateName]: <supabase user id> }` (منسدلة لكل قالب في إعدادات واتساب؛ **أُضيف للسيريالايزر** — فخّ §1.29). عند الرد: `assignee = templateAgents[template] ?? fu.owner_id ?? sent_by` — تُسند إليه **المهمة + محادثة هاتف** (عبر `profiles.hatif_user_id`). `template_name` أُضيف لـSEL في الـwebhook.
