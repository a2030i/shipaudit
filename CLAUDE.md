...
- **حُوِّل حتى الآن**: `portal_lookup` (بوابة العميل — كانت تطالب لارسا بـ79.96 من 6,201.42) و`customer_balance_recon_zoho` (شاشة المطابقة — صارت 171,351.36 = المرجع). **قرار المستخدم: التاجر يرى رصيداً واحداً بلا تفصيل** (أُزيل سطر «رصيد سابق مُرحَّل»).
- **متبقٍ للتحويل (تقرأ الفواتير للإجمالي)**: `customer_money_dashboard` · `zoho_invoice_dashboard` · `legal_escalation_dashboard` · `customer_debt_concentration` · `credit_stop_list` · `working_capital_now` · `next_best_actions` · `hatif_contact_profile` · `hatif_contact_labels` · `hatif_phone_tags` · `campaign_recipient_context`.
