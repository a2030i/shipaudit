...
### 1.35 نظام تذاكر خدمة العملاء — متقاعد ومحذوف (2026-08-28)
- أزيلت مسارات `/ticket` و`/support` ومكوّناتها وصلاحيات `support.*` ومؤشراتها من نبض التشغيل.
- تحقّق الحذف من أن جداول التذاكر والأحداث والمرفقات وحاوية التخزين جميعها فارغة، ثم أزال migration التقاعد الجداول وRPCs وسياسات الوصول. يبقى سجل حاوية Supabase الخاصة الفارغة فقط لأن حذفها المباشر محمي ويتطلب Storage API، ولا يصل إليها التطبيق.
- مطالبات شركات الشحن `/claims` و`audit_claims` مستقلة تمامًا ولا يشملها هذا القرار.
...
- **3 مصادر تسجيل**: (١) **`activityLogger.js`** (fire-and-forget، الفشل صامت) → edge function **`track-activity`** تلتقط IP من `x-forwarded-for` والدولة من `cf-ipcountry` **سيرفرياً** — دخول مرة/جلسة (App) + كل تنقّل (`logPageView` بمنع تكرار المسار المتتالي) + كل محاولة ممنوعة (`logDenied`). (٢) **تصدير**: `persistAndDownloadExport` يسجّل (نقطة العبور الوحيدة §1.13). (٣) **trigger `log_sensitive_change()`** على الجداول الحسّاسة (payments/carrier_operations/audits/period_closes/app_settings + profiles-update) — يلتقط `auth.uid()` من الـDB نفسها فلا يُتجاوز من الواجهة. **لا triggers على جداول الإدراج الجماعي** (cod_settlement/audit_shipments — آلاف الصفوف/رفعة).
...
- **المخرج الرسمي = PDF بهوية لمحة** (طلب المستخدم): `officialPdf.js` يبني صفحة A4 كاملة (ترويسة متدرّجة بألوان الهوية + الشعار الأبيض + جداول الخانات + صندوق الصافي + سطرا توقيع) ويفتحها في نافذة طباعة → «حفظ كـPDF». **لماذا لا jsPDF/pdfmake؟** كلاهما يحتاج تضمين خط عربي كامل (مئات الكيلوبايتات) ويكسر التشكيل واتجاه RTL في الجداول؛ محرّك طباعة المتصفح يرسم العربية مثالياً ويحترم `@page` (نفس نهج §1.21). حاجب النوافذ المنبثقة → ينزّل HTML بدل الفشل الصامت. **مُتحقَّق بصرياً ببيانات حقيقية**: الشعار يُحمَّل · RTL سليم · 3 جداول/20 صفاً · بلا تجاوز أفقي. نسخة Excel تُؤرشَف صامتة في السجل (kinds `vat_return`/`pnl_statement`).
- **الوصول**: بطاقتان في `/reports` (أرباع جاهزة عبر `quarters()` + فترة مخصّصة) **وزرّان في `/pnl` مباشرةً** — الوصول الوحيد كان «مكتبة التقارير» داخل قسم الحملات ولم يجدهما المستخدم. زر الإقرار في `/pnl` يستنتج الربع من الشهر المعروض.
- **ضريبة الربع الجاري حيّة (2026-07-28)**: طلب المستخدم «بدل ما أسحب إكسل كل شوي». جدول `vat_snapshots` (صف/ربع) + `refresh_vat` في الدالة + **cron `zoho-vat-refresh` كل 30 دقيقة** (jobid 18، بمفتاح الكرون) — فالشاشات تقرأ RPC `vat_current_quarter()` **فوراً** بلا استدعاء زوهو البطيء. معروضة في: **الرئيسية** (بطاقة رابعة مع النقد، تنقل لـ/pnl) و**الوضع المالي** (بطاقة كاملة: الصافي + ضريبة المبيعات/المشتريات + المبيعات الخاضعة + آخر تحديث + زر «تحديث الآن» فوري). الرصيد الدائن (صافي سالب) يظهر أخضر بعبارة «لصالحك».
- **قاعدة**: أي تقرير زوهو رسمي جديد يمرّ بـ`zoho-reports` بـ`filter_by=TransactionDate.CustomDate`، **ويُتحقَّق منه بفترتين مختلفتين قبل التسليم** (تطابق النتيجتين = التواريخ مُتجاهَلة). وأي رقم زوهو يُعرض بتكرار = كاش + كرون، لا استدعاء API عند كل فتح شاشة.

### 1.48 أداء الإقلاع — كل الصفحات كسولة + قطع سلاسل المكتبات الثقيلة ✅ (2026-07-28)
بلاغ «الموقع بطيئ جدا جدا جدا». القياس قبل: حزمة واحدة **2,347KB** والـ41 صفحة تُركَّب كلها عند الإقلاع (PageSlot يخفي ولا يفصل §2.1). بعد: **3 ملفات/177KB** على شاشة الدخول (domReady 112ms)، والحزمة الرئيسية 265KB.
- **كل صفحة `React.lazy`** في App.jsx (LoginPage وحدها ثابتة). `PageSlot` يلفّ بـ`Suspense` ويحمل **مرجع `seen`** — الصفحة لا تُركَّب حتى **أول تفعيل** لها، ثم يُجمَّد محتواها وهي مخفية (`frozen` ref) فلا تُعاد رسمها مع كل تنقّل. ملف يُصدِّر صفحتين (Settings.jsx) = `lazy` لكل تصدير عبر `.then(m => ({ default: m.X }))`.
- **سلسلتان ثابتتان كانتا تجرّان مكتبتين ثقيلتين لشاشة الدخول** (مكتشَفتان بالتتبّع لا بالتخمين): `App → Settings.jsx → engine/export.js → xlsx` و`App → AIChat → محلّلا الكشوف → pdfjs`. **مكوّن مركَّب في كل صفحة (AIChat) ممنوع أن يستورد محلّلاً ثقيلاً ثابتاً** — الاستيراد ديناميكي داخل المعالِج.
- **حزمة `core`** (`vite.config.js`): بلا تخصيص صريح يبتلع rollup النواة المشتركة (supabase/UI/permissions/auth/coreService) داخل حزمة `exports` فيضطر ملف الدخول لاستيرادها **ومعها الإكسل**. + `modulePreload.resolveDependencies` يمنع وسم xlsx/pdfjs/exports للتحميل المسبق (`manualChunks` يضع وسم preload لكل chunk معرَّف).
- **الكاشف**: `tools/trace-heavy-imports.cjs` يتتبّع سلسلة الاستيراد **الثابتة** من `src/main.jsx` لأي مكتبة ثقيلة ويطبع المسار كاملاً. شغّله بعد أي استيراد جديد في App/UI/AIChat.
- **الخطوط WOFF2 (2026-07-29)**: بعد قصّ الجافاسكربت تبيّن أن **الخطوط صارت أكبر بند** — ثلاثة ملفات PingAR بصيغة OTF بـ**457KB** على شاشة الدخول (٧٠٪ من الحمل الحقيقي). التحويل لـWOFF2 وفّر **٦٠٪** (457 ← 185KB) بصفر تغيير بصري (نفس الخط، ترميز أكفأ). القاعدة في `index.css`: **`woff2` قبل `otf` في نفس `src:`** — المتصفّح يأخذ أول صيغة مدعومة فلا يُنزَّل OTF إطلاقاً (مُتحقَّق: `otfLoaded = 0`). الأداة `tools/fonts-to-woff2.cjs` (wawoff2 كـdevDependency) تُشغَّل بعد أي تحديث لملفات OTF. **التجزئة العربية (subsetting) توفّر أكثر بكثير لكنها تحتاج fonttools/Python غير المتوفّر** — تُنفَّذ متى توفّرت البيئة.
- **⚠️ فخّ القياس**: `performance.getEntriesByType('resource')` بعد `DOMContentLoaded` **لا يرى الخطوط** (تُطلَب لاحقاً عند أول رسم نصّ). قياسي الأول قال «177KB» وهو JS فقط. **القياس الصحيح: `await document.fonts.ready` + مهلة ~1.5ث، ثم اجمع كل الموارد لا `.js` وحدها.**
- **قاعدتان**: (١) أي صفحة جديدة تُضاف لـApp.jsx **كـ`lazy`** لا استيراداً ثابتاً. (٢) أي وحدة تُستورَد من App.jsx/AIChat/UI **ممنوع أن تجرّ xlsx أو pdfjs ثابتاً** — التصدير/التحليل يُستورَد ديناميكياً عند الفعل. `npx vite build` ثم `tracexlsx.cjs` = التحقّق.

### 1.49 حزمة ما بعد فحص الإنتاج — RLS صامت · فهرس مفقود · صحّة الكرونات ✅ (2026-07-29)
من تقرير فحص القاعدة الحيّة (إحصاءات 95 يوماً + advisors + cron). كل بند مُثبَت بقياس قبل/بعد لا باستنتاج.
- **فخّ RLS الصامت (الأخطر)**: `retargeting_followups` كان **RLS مفعّلاً بصفر سياسات** → أي `select` من موظف يُرجِع **صفراً بلا خطأ** (233 صفاً موجودة). الأثر تعدّى `loadFollowupsMap`: **أربع دوال INVOKER** تقرأه (`hatif_leads_enriched` · `hatif_contact_profile` · `next_best_actions` · `sla_breaches`) كلها فقدت حالة المتابعة. **قاعدة: `RLS enabled + 0 policies` = جدول مخفي لا محمي — افحص `rls_enabled_no_policy` في advisors بعد أي جدول جديد، وتحقّق بـ`set local role authenticated; select count(*)`.** (`ar_aging_snapshots`/`retargeting_snapshot_summary` بلا سياسات أيضاً لكن لا يقرأهما الكود مباشرة — تُركا عمداً؛ `zoho_auth` مقصود.)
- **الفهرس المفقود تحت أثقل استعلام**: `crm_leads` بها 13 فهرساً ولا واحد على `created_at` — وهو عمود الفرز في الاستعلام الذي استهلك **54% من زمن القاعدة**. Seq Scan + فرز خارجي 3.8MB على القرص → 809ms/صفحة؛ بعد `ix_crm_leads_created_desc` → **328ms**، والـRPC 1,558→**425ms**. وحُذف الاحتياطي الذي كان يسحب 94 ألف صف للمتصفّح عند فشل الـRPC + دالتان ميتتان بنفس النمط. **قاعدة: الاحتياطي عند فشل RPC لا يجوز أن يكون تحميلاً كاملاً — الفشل النادر يستحق خطأً ظاهراً.**
- **`cron_health()` — الكرونات كانت عمياء**: `cron.job_run_details` تقول «نجح» دائماً لأنها تقيس **إرسال** الطلب لا نتيجته (pg_net يتوقّف عند 5 ثوانٍ). الواقع: **105 من 250 استجابة/24س «انتهت مهلتها»** بلا إثبات. الدالة تقيس **الأثر في البيانات** (آخر صف كتبته المهمة، أو طابور عالق حان وقته) وتُعرَض في `/integrity`. **فخّ مُتحقَّق: الإشارة diff-based (`hatif_conversation_tags.applied_at`) لا تتحرّك في الساعات الهادئة** — أنتجت إنذاراً كاذباً حتى وُسِّعت العتبة لـ12 ساعة. **قاعدة: أي مهمة جديدة تُضاف لـ`cron_health` بإشارة أثر + عتبة تراعي أن «لا تغيير» ليس عطلاً.**
- **ثغرة `attach_moyasar_payment`**: كانت قابلة للتنفيذ من **anon** وتكتب `paid_at` بمعرّف دفعة مخترَع بلا تحقّق مع ميسر. سُحبت (`payment_requests` **فارغ تماماً** — التدفّق لم يُستخدم قط فلا كسر). **تُعاد الصلاحية فقط مع تحقّق خادم-لخادم بمفتاح ميسر داخل دالة طرفية.**
- **⚠️ فخّ الصلاحيات**: `revoke execute on function … from anon` **لا يكفي** — Postgres يمنح EXECUTE لـ`PUBLIC` افتراضياً وanon يرثها. **السحب يكون `from public` ثم منح صريح** (مُتحقَّق: بعد السحب من anon وحده بقيت `has_function_privilege('anon',…)=true`).
- **سجلّات إضافة-فقط محميّة**: `contract_history` · `internal_export_pulls` · `activity_log` كانت `ALL USING(true)` — أي موظف يمسحها عبر API. صارت قراءة+إضافة للمسجَّلين، وتعديل/حذف للمدير. مُختبَر حياً: `delete … where true` كموظف = **0 صف**. (التطبيق لا يحذف منها إطلاقاً — فُحص كل `.delete()` في `src/lib`.)
- **`hatif_events` سياسة حذف**: كان ينمو **8,045 صفاً/يوم** (~1 جيجابايت سنوياً) بلا حدّ، بينما المستهلك الوحيد يقرأ `AssignedUserIdChanged` بنافذة 30 يوماً و**93% من الأحداث صدى لكتاباتنا** (وسمُنا وخصائصنا تعود كأحداث). `purge_hatif_events()` + كرون يومي (jobid 19): الصدى بعد 14 يوماً والباقي بعد 180.
- **الربط التلقائي صار جزءاً من الرفع**: `autoLinkFromZoho()` (merchantsService) استُخرِج من زر الصفحة ويُستدعى **بعد رفع كشف المتاجر** — كانت خطوة يدوية تُنسى فيبقى العميل الجديد بلا متجر (بلا هاتف ولا سياق) في كل شاشات التحصيل والحملات. فشلها لا يُفشل الرفع.

### 1.50 إيقاف تكامل Gmail + إخراج الأسرار من `app_settings` ✅ (2026-07-29)
من مراجعة تقرير جاهزية خارجي (تحقّقتُ من ٣٥ ادّعاءً: ٣١ صحيح، ٣ متقادمة، وواحد مبالغ في توصيفه).
- **`app_settings` مقروء لكل `authenticated`** — وكان يحمل **`GOOGLE_CLIENT_SECRET`** بنصّه. مُتحقَّق حياً: `set role authenticated; select * from app_settings` أعاد السرّ كاملاً. **قاعدة: `app_settings` للإعدادات المعروضة فقط. أي سرّ خادمي → متغيّر بيئة الدالة، أو جدول `app_secrets`** (RLS بلا سياسات + `revoke all from anon, authenticated` — الموظف يُرفَض بصلاحية الجدول لا بالسياسة).
  - **تمييز مهم عن فخّ §1.49**: «RLS بلا سياسات» **عطل صامت** حين تقرأ الواجهة الجدول (retargeting_followups)، و**هو المطلوب** حين لا تقرأه أصلاً (app_secrets). الفارق: هل للجدول مستهلك في `src/`؟
- **تكامل Gmail أُوقف بالكامل** (قرار المستخدم بعد عرض الدليل): آخر صفّ كتبته المزامنة **6 مايو** (١٢ أسبوعاً بلا إنتاج) · **صفر استدعاء** من الواجهة لأي من دواله أو لجدول `gmail_connections` · والفواتير تصل عبر **InboxDone → webhook-intake** لا Gmail. ومع ذلك كان كرون `gmail-auto-sync-6h` يعمل ٤ مرات يومياً ويُبلّغ «نجاح» — نفس عمى الكرونات (§1.49).
  - نُفِّذ: إيقاف الكرون · حذف `GOOGLE_CLIENT_*` من القاعدة · حذف توكن التحديث المخزَّن · **٥ دوال صارت شواهد قبر 410** (gmail-auth/callback/sync/sync-cron/fetch-attachment).
  - **أُغلق معها نوعان مؤكَّدان من الثغرات** كانا في `gmail-callback`: `state` غير موقّع (base64 فقط) يُؤخذ منه `userId` → **account-linking CSRF**، و`returnUrl` من نفس الـstate يُمرَّر لـ`Response.redirect` بلا قائمة سماح → **open redirect**. مُتحقَّق حياً بعد الإيقاف: طلب بـstate مزوّر وreturnUrl خارجي يردّ **410 بلا أي تحويل**.
  - **الدرس**: تحصين مكوّن ميت هدر ويُبقي سطح الهجوم. اسأل «هل يُنتج شيئاً فعلاً؟» **قبل** ترقيعه — الجواب من البيانات (آخر صفّ مكتوب + مسح `src/` للمستدعين) لا من وجود الكود.
  - **خطوة المستخدم المتبقية**: إلغاء تطبيق OAuth من Google Cloud Console (السرّ كان مقروءاً لكل جلسة موظف فيُعتبر مكشوفاً — الحذف من القاعدة لا يُبطل النسخة القديمة).
- **رفع إيصالات السداد** كان مفتوحاً للزائر المجهول (10MB بلا حدّ عدد ولا ربط بطلب صالح). قُيِّد للمسجَّلين — `payment_requests` **فارغ تماماً** فلا تدفّق انكسر. إعادة الفتح تكون برابط رفع موقّع بعد إنشاء الطلب، لا بفتح anon.
- **قياس الأداء لا يكتمل بعدّ ملفات JS**: قلتُ «١٧٧KB عند الإقلاع» وكان القياس ناقصاً — **خطوط PingAR الثلاثة (OTF) تُحمَّل بعد `DOMContentLoaded` بـ457KB**، أي ٧٠٪ من حمل شاشة الدخول الحقيقي (~657KB). **قاعدة: قِس بعد `document.fonts.ready` + مهلة، وبكل أنواع الموارد لا `.js` وحدها.** تحويلها لـWOFF2 بـsubset عربي = أكبر مكسب أداء متبقٍ.

### 1.51 كنس ما مات — ١١ جدولاً ودالتان ✅ (2026-07-29)
جرد شامل بعد إيقاف Gmail (§1.50): قاعدة البيانات + الكود + التخزين + الحزم.
- **⚠️ التصحيح الذي منع خطأً**: ظننتُ أول الأمر أن صفحة `/tasks` جزء من ذيل Gmail. **ليست كذلك** — هي «مهام الأسبوع» للناقلين وتقرأ `carrier_task_schedules`. جدول `tasks` شيء آخر تماماً: **صندوق بريد** (`email_subject`/`email_from`/`email_body`/`email_html`/`ai_is_financial`) كانت تملؤه مزامنة Gmail **وبلا أي واجهة تقرأه**. **الدرس: تشابه الاسم ليس دليل ارتباط — افحص أعمدة الجدول ومَن يقرؤه، لا اسمه.**
- **حُذف (بيانات فقط، صفر تغيير في الواجهة)**: `tasks` (3,690 إيميلاً · 12MB) · `attachments` (479) · `task_actions` (4,103) · `notifications` · `sync_rules` — الخمسة **بصفر مرجع** في `src/` و`supabase/functions/`، وآخر صفّ 6 مايو. وجداول ميتة بصفر صف وصفر إدراج على الإطلاق: `whatsapp_messages` · `whatsapp_campaigns` · `zoho_events` · `custom_oauth_providers` · `gmail_connections` · `migrations` (جدول هجرات قديم — Supabase له `supabase_migrations`). ودالتان: `get_sender_counts` · `zoho_customer_unused_credits` (استُبدلت بـ`zoho_applicable_credits` §1.26b).
- **⚠️ فخّ حذف التخزين**: `delete from storage.objects` **مرفوض** بحارس `storage.protect_delete()` («Use the Storage API instead») — والأسوأ أنه يُرجِع الهجرة كاملةً. أخرِج حذف التخزين من أي هجرة؛ الحاوية تُحذف من اللوحة أو عبر الـAPI. (`task-files`: 406 ملفاً/52MB تنتظر حذف الحاوية.)
- **ما بدا ميتاً وليس كذلك — لا يُحذف**: الجداول الفارغة قد تكون **ميزات تعمل** (`period_closes` · `audit_claims` · `campaign_queue` · `ivr_queue` · ~~`payment_requests`~~ (حُذف §1.57) · `bad_debt_writeoffs` · `bank_balance_log` · `dispute_notes` · `crm_deals` · `cod_upload_labels` · `hatif_retag_dirty`). **الفراغ ≠ الموت**: `bad_debt_writeoffs` قُرئ 445 مرة — الصفحة تعمل والجدول فارغ فقط. **المعيار الصحيح للحذف = صفر صف + صفر إدراج تاريخي (`n_tup_ins`) + صفر مرجع في الكود + قرار منتج صريح**؛ نظام التذاكر استوفى هذه الشروط وحُذف في §1.35.
- **نظيف بالفعل**: صفر ملف يتيم في `src/` · صفر حزمة npm زائدة (٧ حزم كلها مستعملة) · وكل ملفات الصفحات مستوردة (ما ليس في App.jsx تستورده الهَبات).
- **متبقٍ لخطوة المستخدم**: حذف حاوية `task-files` من لوحة Supabase (52MB) · وحذف **١٠ دوال شواهد قبر** ما زالت ACTIVE (`reset-admin` · `zoho-intake` · `whatsapp-send` · `zoho-debug` · `hatif-probe` · و٥ دوال Gmail) + `lamha-sync` (ping تجريبي).

### 1.52 دين العميل = زوهو بالهللة — الرصيد الافتتاحي + الروابط الميتة ✅ (2026-07-29)
سؤال المستخدم: «ما تقرأ الرصيد الافتتاحي؟». الجواب: لا — وكان الخطأ مزدوجاً.
- **⚠️ القاعدة الجديدة: جمع `zoho_invoices` لا يساوي دين العميل.** الرصيد الافتتاحي المُدخَل في زوهو عند التأسيس يُنشئ ذمّة مدينة **بلا مستند فاتورة**، فيسقط من أي جمع للفواتير. المقاس على الإنتاج: 68 عميلاً مديناً · زوهو **171,351** ومجموع فواتيرنا **112,086** → **59,265 ر.س دين مخفي على 27 عميلاً**.
- **نقطة الحقيقة الواحدة = view `customer_ar`**: الإجمالي `zoho_contacts.outstanding_receivable` (يحسبه زوهو ويشمل كل شيء) + تفصيل الفواتير (`invoiced_due`/`opening_due`/`open_invoices`). **أي شاشة تعرض دين عميل تقرأ منها** — ممنوع جمع `zoho_invoices` للإجمالي (تبقى للتفصيل «من أين جاء الدين» وللأعمار وزاتكا والإشعارات).
- **الفخّ الثاني — الروابط الميتة**: `customer_merchant_links.store_id` قد يشير إلى متجر **اختفى من أحدث كشف** فيظهر العميل بصفر. حالة «لارسا»: **متجران بنفس الاسم** (1466 المختفي ↔ 189 الحي) والرابط على المختفي، فالبوابة كانت تعرض **0.00** بينما دينه 6,201.42. القاعدة الحية: **3 روابط ميتة من 994 تُخفي 10,606 ر.س** — فالعدد ضئيل والأثر كبير.
- **حُوِّلت كلها ✅ (٦ عشرة دالة)**: ~~`portal_lookup`~~ (بوابة العميل — **حُذفت لاحقاً §1.57** — كانت تطالب لارسا بـ79.96 من 6,201.42؛ **قرار المستخدم: التاجر يرى رصيداً واحداً بلا تفصيل**) · `customer_balance_recon_zoho` · `zoho_invoice_dashboard` · `customer_debt_concentration` · `working_capital_now` · `customer_money_dashboard` · `legal_escalation_dashboard` · `credit_stop_list` · `next_best_actions` · `hatif_contact_profile` · `hatif_contact_labels` · `hatif_phone_tags` · `campaign_recipient_context` · `zoho_overdue_campaign` · `capture_ar_aging_snapshot` · `balance_reconciliation`. **مُتحقَّق: كل إجمالي عام = 171,351.36 بالهللة.**
- **قاعدة الشريحة**: الرصيد الافتتاحي **بلا تاريخ فاتورة** → يُوضَع في **+90 يوم** ويُعدّ **متأخراً** بحكم تعريفه (أقدم دين لدى العميل). مطبَّقة في: الأعمار · التصعيد القانوني · الإيقاف الائتماني · تاق «متأخر سداد».
- **الدوال التي تبقى على جمع الفواتير عمداً**: `zoho_applicable_credits` (لا يُطبَّق رصيد دائن إلا على فاتورة قائمة) · `zatca_pending_today` (المستند نفسه) · وتفاصيل الأعمار داخل الدوال المحوَّلة (الإجمالي من `customer_ar`، التوزيع على الشرائح من الفواتير).
- **الدوال المرتبطة بالهاتف/المتجر تُظهر 166,946.86 لا 171,351.36** — الفارق (4,404.50) عملاء **بلا رابط متجر** فلا هاتف لهم. ليس خطأً: من لا رقم له لا يظهر في شاشة اتصال.
- **فحص `zoho_debt_no_invoices` أُعيد تأطيره**: لم يعد «دين مخفي» — صار «رصيد افتتاحي بلا فاتورة تقابله» (مُحتسَب في كل الشاشات، ويبقى بلا مستند حتى يُفوتَر في زوهو).
- **معلّق لقرار المستخدم**: «مؤسسة عبق المنزل — HOME SCENT» دينها 3,405.50 ورابطها يشير لمتجر `1315` المختفي **ولا متجر حيّ بنفس الاسم**.

### 1.53 فخّ المتجر الشبح — «اسم → متجر» يُختار بترتيب نصّي ✅ (2026-07-29)
سؤال المستخدم عن «مؤسسة كوبرا دريل» كشف خللاً بنيوياً في المطابقة، لا حالة فردية.
- **الجذر (سطر واحد)**: `normalize_arabic_name` يجرّد بادئات مثل «مؤسسة/متجر» فيتصادم متجران بالاسم نفسه، و`resolve_snapshot_names` كان يفضّ التعادل بـ`ORDER BY store_id` — و**`store_id` نصّ**، فـ`'1255'` يسبق `'517'` سلسلةً. النتيجة: يفوز **المتجر المسجَّل لاحقاً** (رقمه أكبر بأربع خانات فيسبق نصّياً) وهو دائماً **الشبح**: تسجيل مكرَّر بصفر شحنة · دفع مسبق · غير نشط.
- **القياس على الإنتاج**: 24 اسماً متصادماً، **8 منها اختارت الشبح** — وكلها بلا استثناء صفر شحنة مقابل 122–3,905 شحنة للمتجر الحقيقي.
- **الأثر المالي المؤكَّد**: (١) استحقاق كوبرا دريل 464.60 على متجر شبح 1255 بدل 517. (٢) **«متجر الأندية» — 31,053.70 ر.س** (رصيد افتتاحي بالكامل، أكبر مدين مفرد) كان مربوطاً بشبح 1055؛ ولأن الشبح **«دفع مسبق»** أسقطه فلتر `credit_stop_list` (`billing_type <> 'دفع مسبق'`) → **أكبر مدين لدينا كان غائباً عن قائمة الإيقاف الائتماني**، وحملاته تذهب لهاتف الشبح.
- **نقطة الحقيقة الواحدة**: view **`merchant_by_name`** (`distinct on (nname)` بأولوية **شحنات ← آخر شحنة ← نشط ← store_id**). أي «اسم → متجر» يمرّ منها. **ممنوع فضّ تعادل متجرين بترتيب `store_id`** — نصّي وعشوائي دلالياً.
- **الإصلاح الآلي بقاعدة يقين لا ترجيح**: يُنقل الإسناد فقط حين يكون المتجر الحالي **صفر شحنة** ونظيره **له شحنات** (متجر بصفر شحنة يستحيل أن يتراكم عليه استحقاق). طُبِّق على `customer_merchant_links` (عدا **اليدوية** §1.9) و`store_balances`. **لم يُمَسّ** متجران حقيقيان بشحنات (بوليصه 187/211 · Smart 1646/316 · حناء العطاء 1895/1896) — تلك تحتاج قرار المستخدم.
- **فحصان في `/integrity`**: `store_link_ghost` (رابط على شبح — يبقى فيه اليدوي فقط) · `store_name_collision` (متجران يشحنان بنفس الاسم — يُحسم بالدمج في المنصّة لا عندنا).
- **حالة «Smart» — حُسمت بتأكيد المستخدم (2026-07-29)**: متجران يشحنان بنفس الاسم فاختارت المطابقة عشوائياً و**عكست**. الدليلان اللذان كشفا القلب: (١) **الهاتف** — متجر 1646 يحمل رقم متجر «متجر ايلا» المملوك قطعاً لـ«محل ايلا ليلا للزهور»؛ (٢) **تطابق باليوم** — 1646 انضمّ للمنصّة 2026-02-06 وأول فاتورة لـ«ايلا ليلا - Smart» في زوهو بنفس التاريخ. صُحِّح يدوياً (`manual`): **316 ← ‏مؤسسة اختيارات الغذاء** · **1646 ← محل ايلا ليلا**. الأثر: الصفّان المنفصلان في شاشة المطابقة (500 زوهو على متجر و400 داخلي على آخر) صارا **صفّاً واحداً بفرق حقيقي 100 ر.س**. **القاعدة المستخلَصة: عند تصادم اسمين، المميِّز = الهاتف المشترك مع متجر آخر مؤكَّد، ثم تطابق تاريخ الانضمام مع أول فاتورة زوهو — لا عدد الشحنات.**
- **فخّ جانبي**: `customer_merchant_links.match_method` محكوم بـ`CHECK` بأربع قيم — أي وسم إصلاح جديد يُرفَض. لا تخترع قيمة؛ عدّل القيد أو أبقِ الوسم.
- **دمج تبويبَي المطابقة (نتيجة مباشرة)**: بعد §1.52 صار «العملاء — زوهو API» و«مطابقة لمحة الداخلية» يقرآن **نفس المصدرين**، والفرق الوحيد **المِرساة**. القياس وقت الدمج: مِرساة العميل = 172,153.56 (= المرجع بالهللة) · مِرساة المتجر = 171,154.56 — **الفارق 999 عميل كامل بلا رابط متجر يختفي**. فصارا **عرضين لتبويب واحد** (`/reconciliation`): `👤 حسب العميل` (الافتراضي) · `🏪 حسب المتجر`. الرفع وقسم «غير مرتبط بمتاجر النظام» **مشتركان** بين العرضين، وعرض المتجر يحمل **بانراً إلزامياً** بعدد ومبلغ المخفيّين + زرّ تبديل. المسارات القديمة تهبط على عرضها (`?tab=zoho_live`→العميل · `?tab=customers`→المتجر). **قاعدة: أي عرض مُفهرَس بالمتجر يجب أن يعلن ما يُخفيه — لا إسقاط صامت لعميل بلا متجر.**

### 1.54 أداء الاستعلامات — قِس ثم أصلح ✅ (2026-07-29)
شكوى «الصفحة عالقة على دوّارة». **لم أخمّن — قِستُ كل RPC في التطبيق**، فتبيّن أن البطء محصور في ثلاث دوال، وكلها لنفس السبب الجذري: **عمل على بيانات لا تُقرَأ**.
- **`autolink_balances_by_exact_name` — 14,565ms → 29ms (٥٠٠×)**. صفحة المطابقة تستدعيها **أولاً بالتتابع** قبل أي جلب، فالمستخدم ينتظرها كاملةً. السبب: `CROSS JOIN` بين **كل** صفوف `store_balances` غير المرتبطة (3,121 صفاً من **44 كشفاً قديماً**) و1,542 متجراً بثلاثة `ILIKE '%…%'` = **14 مليون مقارنة نصّية**. والأسوأ أنها **بلا أي ناتج**: الكشف الحالي فيه **صفر** صف غير مرتبط. الإصلاح: حصر النطاق بأحدث كشف لكل مصدر + خروج مبكر حين لا يوجد ما يُربَط. **القاعدة: أي دالة تعمل على «كل الكشوف» تُحصَر بالأحدث — الكشوف القديمة تاريخ لا يُعالَج.**
- **`crm_leads_dashboard_meta` — 2,207ms → 29ms (٧٦×)**، وهي أول ما يُنتظَر في مركز المبيعات (وسجّلت **7.9 ثانية** في الذروة). كانت تُجسِّد 94 ألف صف في CTE ثم تمرّ عليه أربع مرات، والقيم المميّزة بفرز **خارجي على القرص** (4.9 ميغابايت). الإصلاح جزآن: (١) **فهرس مغطٍّ** `ix_crm_leads_meta_cover(status, owner_id, matched_store_id, duplicate_count)` → مسح فهرس فقط بدل 22 ألف صفحة كومة (437→61ms). (٢) القيم المميّزة بـ**مسح تخطّي** (loose index scan عبر recursive CTE) → ١٢ نزولاً في الفهرس بدل فرز 94 ألف قيمة (~35ms، `Heap Fetches: 0`). **مُتحقَّق: الناتج يطابق الجدول الخام حقلاً بحقل.**
- **`integrity_check` — 2,194ms** (منها **1,235ms** لفحص `external_leads_junk`: ستة regex على 94 ألف صف). تُرِك عمداً — صفحة تشخيص نادرة الزيارة، والبوابة تمنع الوهمي عند الرفع أصلاً (§1.37) فالفحص حارس خلفي.
- **الباقي سليم**: `hub_rollup` 140ms · `customer_money_dashboard` 182ms · `working_capital_now` 6ms · `sales_today` 2ms · دوال المطابقة 86ms.
- **القواعد المستخلَصة**: (١) **لا تُحسِّن بالحدس — `pg_stat_statements` + `explain analyze` يحدّدان الجاني في دقيقتين.** (٢) أي عدّاد لوحة على جدول ضخم يحتاج **فهرساً مغطّياً**، وأي قائمة قيم مميّزة تحتاج **مسح تخطّي** لا `distinct` على الجدول. (٣) `CROSS JOIN` مع `ILIKE '%x%'` = قنبلة تربيعية — احصر الطرفين قبلها. (٤) لا تستدعِ دالة كتابة **بالتتابع** على مسار قراءة الصفحة إن لم تكن مضمونة السرعة.

### 1.55 جودة واتساب — فصل «خنق ميتا» عن الفشل التقني ✅ (2026-07-29)
من تدقيق خارجي: «فشل 36.6%». **الرقم صحيح بالضبط** (2,489 من 6,799 في 7 أيام) لكنه **رقم واحد يخفي ثلاثة معانٍ**، والقياس الفعلي كشف أن **الفشل التقني = صفر**:
- **خنق ميتا** (`healthy ecosystem`) 1,222 — حكم واتساب على **استهدافك ومحتواك**. تراكمه يخفّض تصنيف جودة رقمك ثم حدّ إرسالك اليومي وقد ينتهي بحظر. **هذا الإنذار الحقيقي.**
- **غير قابل للتسليم** 1,158 — الرقم بلا واتساب (جودة قائمة الأرقام).
- **تجربة لدى المنصة** 109 — خارج سيطرتنا.
- **RPC `whatsapp_quality(p_dim, p_days)`** (`'template'`/`'campaign'`) يفصلها ويُصدر حكماً. **العيّنة دون 30 رسالة لا يُحكَم عليها** (ضوضاء إحصائية). القياس عند البناء كان قاطعاً:

| القالب | أُرسل | وصلت | ردّوا | خنق ميتا |
|---|---|---|---|---|
| `cash_back5` (دليل خارجي بارد) | 6,451 | 55.5% | **1.8%** | **18.0%** |
| `salla_store` (دليل خارجي بارد) | 1,367 | 54.9% | **1.9%** | **21.7%** |
| `retarget_stopped` (تجّارنا المتوقّفون) | 291 | 73.9% | **13.5%** | **0.0%** |

**الاستنتاج التشغيلي**: القصف الجماعي لأرقام الدلائل الباردة هو ما يحرق تصنيف رقمك. نفس النمط في بُعد الحملة (كل حملات «جهات محتملة» 20–22% خنق).
- **⚠️ تصحيح صياغة (مراجعة خارجية)**: قلتُ «سبعة أضعاف العائد» — وهو **خطأ مصطلح**. المقيس **معدّل ردّ** لا عائد مالي، ولا يصير عائداً حتى يُربَط بفرصة مؤهَّلة أو مبيعة. والنسبة تختلف بالمقام: **الردّ ÷ الموصَّلة** = 13.5% مقابل 1.8% (7×) · **الردّ ÷ المُرسَلة** = 10.0% مقابل 1.0% (10×). وكلاهما **ملوَّث بالجمهور**: القالبان يخاطبان جمهورين مختلفين، فالفرق ليس حكماً على صياغة القالب وحدها. **القاعدة: سمِّ المؤشر «معدّل الردّ» بمقامه الصريح، ولا تسمّه عائداً حتى يُقاس التحويل.**
- **العرض في مكانين**: جدول «جودة القوالب» في تاب سجل الحملات · و**شارة الحكم داخل مودال الإرسال لحظة اختيار القالب** (تحذير بلا منع — القرار للمستخدم لكن لا يُطلق وهو يجهل).
- **حارس الإرسال وُسِّع** (`no_whatsapp_phones`): أُضيف «فشل غير-قابل-للتسليم مرتين فأكثر بلا أي تسليم بعد أول فشل». **الفصل مقصود**: `v_no_whatsapp` تبقى **خاصية العميل** (تُعرَض في هاتف)، و`no_whatsapp_phones()` وحدها **قرار الإرسال**. أضاف **صفر رقم اليوم** (حارس استباقي بلا مخاطرة حظر زائد) — الـ35 رقماً المتسرّبة (27–28 يوليو) كان لكلٍّ منها فشل **واحد** سابق، فتُحجَب عند الثاني.
- **كرون `hatif-contacts-audit`** (jobid 20، 05:35 KSA): `hatif_unknown_contacts` كان يملؤه **زرّ يدوي فقط** فتجمّد عند 21 يوليو — أي رقم كلّمنا بعده لا وجود له في «فرص هاتف». بعد أول تشغيل: **1,943 → 6,694** (4,751 جهة كانت مخفية).
- **قاعدة**: أي مقياس فشل إرسال يُعرَض **مفصولاً بمصدره** (منصّة/رقم/تقني) — «فشل» مجمّعاً يخفي الإشارة الوحيدة القابلة للتصرّف.
- **v2 — تصحيحان من مراجعة خارجية (نفس اليوم، كلاهما مؤكَّد بالقياس)**:
  - **حكم «أرقام رديئة» كان بلا دليل**: قالب `sadad` (33 رسالة · تسليم 18.2% · فشل **صفر** · غير-قابل-للتسليم **صفر**) حُكم عليه بالرداءة، والحقيقة أن **81.8% منها معلّقة بلا حالة ملتقَطة** (webhook هاتف غير مضبوط). **انخفاض التسليم وحده ليس دليلاً على رداءة الجمهور** — الدليل `undeliverable`. أُضيفت خانتا `pending`/`experiment` وحكم **«بيانات غير مكتملة»** (pending ≥ 40%)، و«أرقام رديئة» صارت مشروطة بـ`undeliverable ≥ 25%`.
  - **إنذار كاذب شحنتُه**: «تجربة لدى المنصة» (109 رسالة) كانت ضمن `other_fail` فتُظهر الواجهة «⚠️ فشل تقني» — وهي رفض من ميتا. صارت خانة مستقلة و`other_fail` = **الفشل التقني وحده** (وهو صفر فعلاً).
- **⚠️ تصحيح جوهري لقاعدة §1.49 عن الصلاحيات** (ثلاث محاولات قبل الإغلاق — كلها مقيسة):
  - **مصدرا وصول مختلفان حسب عمر الدالة**: الدوال **الأقدم** تحمل منحاً صريحاً لـ**PUBLIC** (`=X/postgres` في `proacl` — المستفيد الفارغ = PUBLIC) وanon يرثه؛ والدوال المُنشأة بعد ضبط الـdefault ACL تحمل `anon=X` مباشرة. فالسحب من واحد فقط يترك الآخر: أول تمريرة أغلقت **3 دوال من 23**.
  - **⚠️ الجذر الحقيقي — `IN SCHEMA` هو الخطأ**: `ALTER DEFAULT PRIVILEGES … **IN SCHEMA public** REVOKE EXECUTE … FROM PUBLIC` **لا يستطيع إزالة منح PUBLIC الافتراضي العالمي** — دلالة **موثّقة في PostgreSQL**. لذلك بدا `pg_default_acl` نظيفاً بينما الدالة الجديدة تولد بـ`=X`. **العلاج سطر واحد بلا `IN SCHEMA`**: `alter default privileges for role postgres revoke execute on routines from public;` ثم المنح الصريح داخل `public` لـ`authenticated, service_role`. **مُثبَت بالاختبار والحارس معطَّل**: دالة جديدة تولد `anon=false` وبلا `=X`.
  - **الـevent trigger أُنشئ ثم حُذف**: كان علاجاً لعَرَض دلالةٍ لم أقرأها، وحمل مخاطر حقيقية (`WHEN OTHERS THEN NULL` يبتلع فشل السحب صامتاً · استثناء بالاسم لا بالتوقيع فيشمل أي overload · لا يستبعد `in_extension` · مشغّل DDL على القاعدة كلها يتداخل مع مهاجرات Supabase والامتدادات). **الدرس: عندما يخالف السلوك التوقّع اقرأ الدلالة الموثّقة قبل بناء طبقة تحايل عليها.**
  - **القاعدة العملية**: الافتراضيات مضبوطة عالمياً فأي RPC جديد يولد مقفلاً بلا عمل إضافي · ولأي دالة **قائمة** `revoke execute … from public, anon` (الاثنان معاً) · والتحقّق دائماً بـ`has_function_privilege('anon', …)` لا بقراءة الأمر المكتوب.
- **تدقيق كل RPC متاح للزائر (55 دالة)**: بالقياس الحيّ `set role anon` تبيّن أن **RLS صامد** — كل دوال البيانات أعادت **صفر صف** للمجهول. ومع ذلك سُحبت الصلاحية دفاعاً بالعمق (أخطرها `assistant_readonly_sql`: منفّذ SQL حرّ يكشف وجود الجداول ويسمح بالتحسّس). **الباقي حينها: دالتان عامّتان** (`portal_lookup`/`get_payment_config`) — ⛔ [تاريخي — البوابة أُلغيت §1.57]  **حُذفتا مع البوابة، فلم تبقَ أي دالة تطبيق متاحة لـ`anon`**. وما عداهما دوال امتداد pg_trgm تخدم الفهارس والمعاملات.
- **مزامنة المهاجرات**: كل مهاجرات اليوم (**22**) صارت في `supabase/migrations/` بأسمائها الإنتاجية (`<version>_<name>.sql`) فالقاعدة قابلة لإعادة البناء منها. **الفجوة التاريخية باقية**: الإنتاج **260** مهاجرة والمستودع يعيد إنتاج **22** منها فقط بدقّة (الباقي أسماء يدوية لا تطابق الإصدارات) — تُسحَب في مهمة مستقلة.

### 1.57 إلغاء بوابة التاجر للدفع بالكامل ✅ (2026-07-29)
قرار المستخدم: «ماعاد احتاجها، حتى ميسر ماعاد نحتاجها». التحصيل يتم عبر **حملات واتساب + التحويل البنكي المباشر**، فالبوابة والدفع الإلكتروني زائدان — ويسقط معهما ملف §1.56 كاملاً بعد ساعة من بنائه (وهو المسار الصحيح: البنية الأمنية كانت تعالج مخاطرة ميزة لم تعد مطلوبة).
- **حُذف**: صفحة `/portal` (`CustomerPortal.jsx`) · شاشة «طلبات السداد» (`PaymentRequests.jsx` + تبويبها في هَب الأموال) · `paymentRequestsService.js` · تبويب «الدفع الإلكتروني» ومفتاح ميسر من الإعدادات · 4 مفاتيح صلاحية `payment_requests.*` · قواعد CSS لـ`.portal-root` · دوال `portal_lookup`/`get_payment_config`/`attach_moyasar_payment`/`portal_issue_token`/`portal_redeem_token`/`portal_data_for_customer` · جداول `portal_access_tokens`/`portal_access_log`/`portal_otp`/`payment_requests` · الدالة الطرفية `portal-access` (شاهد قبر 410).
- **بلا أي فقدان بيانات — مُتحقَّق قبل الحذف**: `payment_requests` كان **صفر صف** (التدفّق لم يُستخدَم قط) · جداول البوابة كانت صفوف اختبار اليوم فقط · حاوية `payment-receipts` **صفر ملف**.
- **⚠️ ما لم يُمَسّ (تمييز حرج)**: `payments` و`payment_allocations` — **دفعات الناقلين، عصب المحاسبة**، ولا علاقة لها بالبوابة رغم تشابه الاسم. ومفاتيح `payments.*` باقية كما هي.
- **أثر جانبي مطلوب**: `PUBLIC_PATHS` صارت **فارغة** — لا صفحة في الواجهة تُفتَح بلا تسجيل دخول، و**صفر RPC تطبيق متاحة لـ`anon`** (لم تبقَ إلا دوال امتداد pg_trgm). أي سطح عام جديد يُضاف لـ`PUBLIC_PATHS` **مع توثيق سبب كونه عاماً**.
  - **⚠️ تصحيح صياغة**: قلتُ «سطح الهجوم الخارجي صار صفراً» — **خطأ**. الصحيح **«صفر RPC تطبيق متاحة لـanon»** فقط. يبقى **26 دالة طرفية بـ`verify_jwt=false`** لأغراض مشروعة (webhooks زوهو/هاتف/Voxa + الكرونات) — كلٌّ منها محروسة بمفتاحها لا بالمصادقة، وهي سطح خارجي قائم. **قاعدة: لا تُعمَّم نتيجة قياسٍ محدود على الأمان كلّه.**
- **⚠️ فخّ `drop policy if exists` باسم مفترَض**: ترحيل الحذف حاول إسقاط سياستَي حاوية الإيصالات بأسماء **خمّنتُها** (`payment_receipts_*`) بينما الفعليتان «Team read payment receipts» و«Upload payment receipts». و`if exists` لا يفشل على اسم غير موجود، **فمرّ الترحيل بنجاح كاذب والسياستان باقيتان** حتى كشفهما تدقيق خارجي. **القاعدة: اقرأ `pg_policies` قبل الحذف — `if exists` مفيد للتكرار وخطر على التحقّق.**
- **خطوة المستخدم المتبقية**: حذف حاوية `payment-receipts` (صفر ملف) والدالة `portal-access` من **لوحة Supabase** — `storage.protect_delete()` يحرس `storage.buckets` أيضاً لا `objects` فقط (تصحيح لما ظننتُه في §1.51)، ولا واجهة MCP لحذف دالة طرفية. حتى ذلك: السياستان محذوفتان والحاوية خاصة، والدالة شاهد قبر 410 بلا أي منطق.

### 1.56 بوابة العميل برابط موقَّع — إثبات هوية بالحيازة لا بالمعرفة ⛔ (2026-07-29، أُلغيت بعد ساعة — انظر §1.57)
`portal_lookup(phone)` كانت تكشف ذمّة أي تاجر **لمن يعرف رقمه** — بلا OTP ولا حدّ معدّل. قرار المستخدم: **رابط وصول آمن ضمن رسالة التحصيل + OTP احتياطي**.
- **الرمز**: 32 بايت عشوائية → base64url (43 حرفاً). **لا يحوي جوالاً ولا مبلغاً ولا معرّف عميل** (مُختبَر). **لا يُخزَّن إطلاقاً — `sha256` فقط**، فتسريب القاعدة لا يمنح وصولاً.
- **دورة الحياة**: صالح 72 ساعة · مقيَّد بعميل واحد وغرض `portal_access` · قابل للإلغاء · عند أول فتح يتحوّل لـ**جلسة 30 دقيقة** ثم يُغلق. **تحديث الصفحة داخل الجلسة لا يقطعها** (وإلا كسرنا الاستخدام العادي)، وبعد انتهائها يُرفَض نهائياً.
- **الجلسة كوكي `HttpOnly + Secure + SameSite=Strict`** — لا يقرؤه JS فسرقة XSS لا تكفي. والرمز **يُمسَح من شريط العنوان فور الفتح** (`replaceState`) فلا يبقى في سجل المتصفّح ولا يتسرّب في Referer.
- **حدّ المعدّل**: 20 محاولة/10 دقائق لكل IP · **وسجل تدقيق لكل نتيجة** في `portal_access_log` (`opened|expired|used|revoked|not_found|rate_limited`) — قراءته للمدير فقط.
- **مُختبَر حياً (٦ مسارات)**: رمز صالح ✅ · رمز مزوَّر `invalid` · منتهٍ `expired` · ملغى `revoked` · مستهلَك بعد الجلسة `used` · 25 محاولة `rate_limited`. والدالة الطرفية `portal-access` (verify_jwt=false) أعادت 482.90 لكوبرا دريل مع الكوكي الصحيح.
- **الجداول الثلاثة** (`portal_access_tokens`/`portal_access_log`/`portal_otp`): RLS مفعّل **بلا سياسات كتابة** + `revoke all from anon, authenticated` — المسّ عبر دوال DEFINER أو service_role فقط (نمط §1.50). ودوال الرمز **كلها service_role**: الزائر لا يملك إلا تمرير الرمز للدالة الطرفية.
- **⚠️ فخّ مؤكَّد**: `pgcrypto` في مخطط `extensions` لا `public`، و`search_path` المقيَّد لدوال DEFINER لا يراه. **الحل تأهيل الاستدعاء `extensions.digest(...)` لا توسيع الـsearch_path** — توسيعه في دالة DEFINER يفتح باب حقن المخطط.
- **🟡 المتبقّي قبل سحب `anon EXECUTE` من `portal_lookup`** (بهذا الترتيب — سحبها الآن يقطع التاجر): (١) إدراج الرابط في رسائل التحصيل (`portal_issue_token` عند بناء الحملة) · (٢) OTP الاحتياطي (5 دقائق · محاولات محدودة · مهلة إعادة إرسال · hash فقط — الجدول جاهز) · (٣) اختبار مع تاجر حقيقي · (٤) **ثم** السحب. **القاعدة: لا يبقى البحث الحرّ بالجوال بعد اكتمال المسار الجديد.**

### 1.58 استبعاد «يكلّمهم الفريق» كان يحجب 99% بلا سبب ✅ (2026-07-29)
سؤال المستخدم «ليش ما ترسل لـCOOMO WATCHES؟» كشف خللاً في قاعدة §1.41.
- **الخلل**: `hatif_touched_phones` كانت تستبعد كل رقم **أُسنِدت محادثته لموظف خلال 30 يوماً** — **بلا نظر إلى أنها أُغلقت**. والمحادثات تُغلق بعد دقائق. القياس الحيّ: **429 محادثة أُسنِدت خلال 30 يوماً · 425 منها مُغلقة · 4 فقط مفتوحة** — أي **99% حجب زائد**.
- **الحالة الكاشفة**: COOMO WATCHES (متجر 101 · دين **4,430.49**) — محادثته أُسنِدت 26 يوليو 10:05 ص و**أُغلقت 10:20 من اليوم نفسه** (15 دقيقة)، وبقي محجوباً عن حملات التحصيل ثلاثة أيام. والمستخدم قرأها **«رفض إرسال»** لأن المودال يقول «استُبعد 1» بلا مَن ولا لماذا.
- **الإصلاح**: الاستبعاد **فقط للمحادثة المفتوحة فعلاً** — لا يوجد `StatusChanged` بـ`status=2` **بعد** آخر إسناد. وهو المعنى المقصود أصلاً: لا تقاطع حواراً **جارياً**، لا كل من كلّمناه يوماً. النتيجة: **المستبعَدون 429 → 3**.
- **دلالة الحالة** (Voxa يرسلها رقماً لا نصاً §1.40): `1` = مفتوحة (210 حدثاً) · `2` = مغلقة (5,249) — مستنتَجة من التوزيع الحيّ ومن تسلسل الأحداث (تُفتح ← تُسنَد ← تُغلق).
- **قاعدتان**: (١) **أي استبعاد بنافذة زمنية يجب أن يقيس الحالة الراهنة لا مجرّد وقوع الحدث** — «أُسنِدت» ليست «ما زالت مفتوحة». (٢) **أي استبعاد صامت يجب أن يُظهر مَن ولماذا** — سطر «استُبعد N» بلا تفصيل يُقرأ عطلاً لا سياسة.
...
- **العملاء** (`customers` — دمج التحصيل + المبيعات): مركز التحصيل · ملف العميل الموحد · مركز المبيعات · CRM.
