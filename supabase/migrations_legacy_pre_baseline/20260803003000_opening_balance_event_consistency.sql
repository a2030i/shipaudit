-- الرصيد الافتتاحي في مؤسسة Zoho الحالية هو فاتورة خاصة بتاريخ 2026-01-10.
-- نحفظ نوعها، ونستعيد أحدث لقطة رصيد عميل من أحداث الفواتير الأحدث من مرآة contacts.
alter table public.zoho_invoices add column if not exists invoice_type text;

update public.zoho_invoices invoice
set invoice_type = event.payload->'invoice'->>'type'
from public.zoho_webhook_inbox event
where event.entity_type = 'invoice'
  and event.entity_id = invoice.zoho_id
  and event.payload->'invoice'->>'type' is not null;

with latest_balance as (
  select distinct on (event.payload->'invoice'->>'customer_id')
    event.payload->'invoice'->>'customer_id' customer_id,
    (event.payload->'invoice'->'contact'->>'customer_balance')::numeric customer_balance,
    coalesce((event.payload->'invoice'->'contact'->>'unused_customer_credits')::numeric, 0) unused_credits,
    coalesce(event.provider_modified_at, event.received_at) balance_at
  from public.zoho_webhook_inbox event
  where event.entity_type = 'invoice'
    and event.status = 'processed'
    and event.payload->'invoice'->>'customer_id' is not null
    and event.payload->'invoice'->'contact'->>'customer_balance' is not null
  order by event.payload->'invoice'->>'customer_id',
           coalesce(event.provider_modified_at, event.received_at) desc
)
update public.zoho_contacts contact
set outstanding_receivable = latest.customer_balance,
    unused_credits_receivable = latest.unused_credits,
    last_modified = latest.balance_at,
    synced_at = now()
from latest_balance latest
where contact.zoho_id = latest.customer_id
  and latest.balance_at >= coalesce(contact.last_modified, '-infinity'::timestamptz);
...
as $function$
  ...
  inv_agg as (
    select o.customer_name,
      ...
      count(*)::int inv_cnt, max(o.age) oldest_days,
  ...
  cust as (
    select ar.contact_name customer_name, ar.total_due owed,
      coalesce(i.overdue_amt,0) + ar.opening_due overdue_amt,
      coalesce(i.inv_cnt,0) inv_cnt,
      greatest(coalesce(i.oldest_days,0),
        case when ar.opening_due > 0.5 then (current_date - date '2026-01-10')::int else 0 end) oldest_days,
      coalesce(i.b0,0) b0, coalesce(i.b1,0) b1, coalesce(i.b2,0) b2,
      coalesce(i.b3,0) + ar.opening_due b3,
      ar.opening_due opening_balance
    from customer_ar ar left join inv_agg i on i.customer_name=ar.contact_name
