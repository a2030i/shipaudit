  ...
  for (const att of atts) {
    try {
      ...
      if (att.size > MAX_SIZE) {
        // Too large: store reference only, download on demand
        if (att.attachmentId) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    let userId: string;
    if (isCronCall) {
      // Called by gmail-sync-cron — user_id comes from body
      ...
      // Re-attach body fields so they're available below
    ...
    (async () => {
      try {
        ...
        // Load sync config unconditionally — processOne closes over these in both fresh and resume paths
        ...
        // Load all non-admin profiles once — used for both assign_role and assign_employee lookups
        ...
        if (resume && conn.sync_state) {
          ...
          // Fresh sync: fetch all IDs and classify
          let query = 'in:inbox';
          ...
          // Only check tasks that actually have attachment flags — avoids querying 3000+ irrelevant IDs.
          // Use chunk size 50 (UUID = 36 chars; 500 UUIDs ≈ 18 KB URL → silent Supabase failure).
          ...
          for (const msgId of messageIds) {
            ...
            if (!ex) {
              ...
              // Only re-process if attachment flags say yes but no usable record exists
        ...
        // Mutable remaining — updated as tasks finish for progressive checkpointing
        ...
        const processOne = async (msgId: string) => {
          ...
          // Always remove from remaining after processing (success or failure)
        ...
        for (let i = 0; i < toProcess.length; i += BATCH) {
          ...
          // Save checkpoint after every batch so a timeout preserves progress
        ...
        // Final: how much work remains (should be 0 if all fit in this run)
        ...
        // Save blocked IDs from this run
