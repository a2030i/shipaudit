...
async function sendStaffTemplate(result: Record<string, any>) {
  ...
  const vars = [result.lead_name, result.lead_phone, result.city || 'غير محددة',
    result.category || 'غير محدد', result.campaign_name || 'حملة إعلانية', leadUrl];
...
Deno.serve(async (req: Request) => {
  ...
  const eventId = String(payload.event_id || payload.response_id || payload['معرف الاستجابة'] || '').trim();
