...
Deno.serve(async req=>{
 ...
 const {data:run,error:runError}=await db.from('work_agent_runs').insert({agent_id:agent.id,status:'succeeded',trigger_type:cron?'schedule':'manual',finished_at:new Date().toISOString(),checked_count:Number(snapshot.overdue_invoices||0)+Number(snapshot.new_leads_today||0)+Number(snapshot.overdue_tasks||0),action_count:1,summary,details:{report_type:'management_12h',snapshot,delivery:'in_app'},approved_by:userId}).select('id').single();if(runError)return json({ok:false,error:runError.message},500);
 const now=new Date();const nextRun=new Date(now);const utcHour=now.getUTCHours();
 if(utcHour<6)nextRun.setUTCHours(6,0,0,0);else if(utcHour<18)nextRun.setUTCHours(18,0,0,0);else{nextRun.setUTCDate(nextRun.getUTCDate()+1);nextRun.setUTCHours(6,0,0,0);}
