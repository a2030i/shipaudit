...
import { riyadhDateKey } from '../_shared/idempotency.ts';
...
Deno.serve(async (req) => {
  ...
  const { data: agent, error: agentError } = await db.from('work_agents').select('*').eq('agent_key', 'management_daily_report').single();
  ...
  const { data: snapshot, error } = await db.rpc('management_daily_snapshot');
  ...
  const runKey = `work-agent:management-daily:${riyadhDateKey(now)}`;
  const { data: run, error: runError } = await db.rpc('create_idempotent_work_agent_run', {
    ...
    p_checked_count: Number(snapshot.overdue_invoices || 0) + Number(snapshot.new_leads_today || 0) + Number(snapshot.overdue_tasks || 0),
    p_action_count: 1,
    p_failed_count: 0,
    p_summary: summary,
    p_details: { report_type: 'management_daily', snapshot },
