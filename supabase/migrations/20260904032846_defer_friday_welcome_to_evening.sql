-- Preserve Friday morning Lamha audiences while deferring delivery to the
-- configurable Friday evening batch. All calculations use Saudi time.

create or replace function private.automation_delivery_at(
  p_schedule jsonb,
...
as $function$
declare
  v_delay integer := greatest(coalesce((p_schedule->>'delayMinutes')::integer, 10), 0);
  v_start time := coalesce(nullif(p_schedule->>'sendWindowStart', '')::time, time '09:00');
  v_end time := coalesce(nullif(p_schedule->>'sendWindowEnd', '')::time, time '20:00');
  v_friday_cutoff time := coalesce(nullif(p_schedule->>'fridayMorningCutoff', '')::time, time '12:00');
  v_friday_deferred time := coalesce(nullif(p_schedule->>'fridayDeferredUntil', '')::time, time '18:00');
  v_local_now timestamp := p_reference at time zone 'Asia/Riyadh';
  v_local_target timestamp;
begin
  if coalesce((p_schedule->>'deferFridayMorning')::boolean, false)
     and extract(isodow from v_local_now) = 5
     and v_local_now::time < v_friday_cutoff then
    v_local_target := v_local_now::date + v_friday_deferred;
  else
    v_local_target := v_local_now + make_interval(mins => v_delay);
  ...
  return v_local_target at time zone 'Asia/Riyadh';
end;
$function$;

revoke all on function private.automation_delivery_at(jsonb, timestamptz)
  from public, anon, authenticated;
grant execute on function private.automation_delivery_at(jsonb, timestamptz)
  to service_role;

comment on function private.automation_delivery_at(jsonb, timestamptz) is
  'Computes automation delivery in Saudi time. A Friday morning audience may be retained and deferred to the configured evening batch.';

create or replace function private.validate_automation_schedule_policy()
returns trigger
language plpgsql
set search_path = ''
as $function$
...
begin
  if new.rule_key = 'welcome_new_customer'
     ...
     and coalesce((new.schedule_config->>'deferFridayMorning')::boolean, false) then
    begin
      v_start := nullif(new.schedule_config->>'sendWindowStart', '')::time;
      v_end := nullif(new.schedule_config->>'sendWindowEnd', '')::time;
      v_cutoff := nullif(new.schedule_config->>'fridayMorningCutoff', '')::time;
      v_deferred := nullif(new.schedule_config->>'fridayDeferredUntil', '')::time;
    exception when others then
      raise exception 'welcome_automation_invalid_schedule_time';
    end;

    if v_start is null or v_end is null or v_start >= v_end then
      raise exception 'welcome_automation_invalid_send_window';
    end if;

    if v_cutoff is null or v_deferred is null
       or v_deferred <= v_cutoff
       or v_deferred < v_start
       or v_deferred > v_end then
      raise exception 'welcome_automation_invalid_friday_deferral';
    end if;
  end if;
...
drop trigger if exists automation_rules_validate_schedule_policy
  on public.automation_rules;
create trigger automation_rules_validate_schedule_policy
  before insert or update on public.automation_rules
  for each row execute function private.validate_automation_schedule_policy();
...
begin
  ...
  v_scheduled_at := private.automation_delivery_at(v_rule.schedule_config, now());
  ...
  set
    ...
    details = jsonb_build_object(
      ...
      'deliveryPolicy', case
        when coalesce((v_rule.schedule_config->>'deferFridayMorning')::boolean, false)
          and extract(isodow from (now() at time zone 'Asia/Riyadh')) = 5
          and (now() at time zone 'Asia/Riyadh')::time
            < coalesce(nullif(v_rule.schedule_config->>'fridayMorningCutoff', '')::time, time '12:00')
        then 'friday_morning_deferred'
...
comment on function public.queue_lamha_welcome_automation(text) is
  'Service-role only. Queues one approved welcome per normalized phone in the accepted Lamha snapshot and defers Friday morning delivery according to the editable rule schedule.';


update public.automation_rules
set
  schedule_config = schedule_config || jsonb_build_object(
    'deferFridayMorning', true,
...
select
  ...
  'منع إرسال ترحيب صباح الجمعة وتأجيل جمهور الفحص نفسه إلى دفعة السادسة مساءً'
