-- بوابة العميل كانت تعرض للتاجر **مجموع فواتيره المفتوحة** كدين — والصحيح
-- إجمالي ذمّته في زوهو. الفارق = الرصيد الافتتاحي (ذمّة بلا مستند فاتورة).
-- مثال حي: «لارسا» كانت البوابة تطالبه بـ79.96 ودينه الفعلي **6,201.42**.
-- على مستوى القاعدة: 59,265 ر.س مخفية على 27 عميلاً.
--
-- الآن الإجمالي من `customer_ar` (نقطة الحقيقة)، والفواتير تبقى تفصيلاً،
-- ويُضاف سطر «رصيد سابق مُرحَّل» حين يوجد فرق — كي يرى التاجر مصدر كل ريال
-- ولا يواجه رقماً بلا تفسير.
...
BEGIN
  ...
  store_blob AS (
    SELECT
      ...
      -- ✅ الإجمالي من نقطة الحقيقة (يشمل الرصيد الافتتاحي)
      COALESCE((
      ...
      COALESCE((
        SELECT round(SUM(ar.opening_due)::numeric, 2) FROM customer_ar ar
        WHERE ar.contact_name IN (
          SELECT l.customer_name FROM customer_merchant_links l WHERE l.store_id = m.store_id)
      ), 0) as opening_due,
      COALESCE((
  ...
  SELECT jsonb_build_object(
    ...
    'stores', COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'total_due',      total_due,
      'opening_due',    opening_due,        -- رصيد سابق مُرحَّل (بلا فاتورة)
      -- الفواتير + سطر الرصيد السابق حين يوجد، كي يفسَّر كل ريال
      'invoices',       CASE WHEN opening_due > 0.5
                          THEN invoices || jsonb_build_array(jsonb_build_object(
                                 'id', 'opening', 'number', 'رصيد سابق مُرحَّل',
                                 'date', null, 'amount', opening_due))
                          ELSE invoices END
    )), '[]'::jsonb)
  ) INTO result
  FROM store_blob;
