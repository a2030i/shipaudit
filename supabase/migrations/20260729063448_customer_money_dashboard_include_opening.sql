-- «فلوسي عند العملاء»: `owed` صار الذمّة الكاملة من `customer_ar` (شاملة
-- الرصيد الافتتاحي). والرصيد الافتتاحي بلا تاريخ فيُوضَع في شريحة **+90**
-- لأنه أقدم دين لدى العميل بحكم تعريفه (§1.52).
-- والعملاء الذين كل دينهم افتتاحي كانوا يختفون من الشاشة تماماً — الآن
-- يظهرون (FULL OUTER على قائمة الفواتير).
...
as $function$
  ...
  open_inv as (
    select customer_name, balance, lower(coalesce(status,'')) st, (current_date - date)::int age
  ...
  -- ✅ الذمّة الكاملة من نقطة الحقيقة + الافتتاحي في شريحة +90
  cust as (
    select ar.contact_name as customer_name,
      ar.total_due                                        owed,
      coalesce(i.overdue_amt,0) + ar.opening_due          overdue_amt,
      coalesce(i.inv_cnt,0)                               inv_cnt,
      greatest(coalesce(i.oldest_days,0), case when ar.opening_due > 0.5 then 91 else 0 end) oldest_days,
  ...
  cust_full as (
    select distinct on (c.customer_name) c.*,
      ...
      m.phone, m.store_name, m.billing_type, m.platform_status, m.wallet_balance, m.last_shipment_at
    from cust c
  ...
  select json_build_object(
    ...
    'customers', (select coalesce(json_agg(json_build_object(
        'name', cf.customer_name, 'store_name', cf.store_name, 'phone', cf.phone,
