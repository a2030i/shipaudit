-- Reconciliation is exact to the stored cent.  A difference of 0.01 SAR is a
-- real reconciliation exception; it must never be hidden behind a tolerance.
...
as $function$
  ...
  select j.anchor, j.sid, m.store_name, m.phone, m.billing_type, m.status, m.wallet,
    ...
    case
      when round((j.ibal - j.zbal)::numeric, 2) = 0 then 'matched'
...
revoke execute on function public.customer_balance_recon_zoho() from public, anon;
grant execute on function public.customer_balance_recon_zoho() to authenticated, service_role;

comment on function public.customer_balance_recon_zoho() is
  'Read-only Lamha statement versus Zoho accounting outstanding reconciliation. Exact to 0.01 SAR; no tolerance.';
