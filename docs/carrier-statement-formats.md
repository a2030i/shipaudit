...
**ملاحظة أرامكس V2:** `Total Balance = SUM(amount موقّع)` (الأقواس = دائن).
نشتق dr/cr من الإشارة، docType = RV (موجب) / DG (سالب)، shipmentType من
اللاحقة. مُتحقَّق: 12 عملية = 42,130.92 ✓.

---

## 5. المطابقة (Reconciliation)

### 5.1 مطابقة على مستوى الرصيد ✅ (المبني)
عند استيراد أي كشف، `CarrierStatements.jsx` يعرض بانر يقارن:
- **رصيد الكشف** (`totals.totalBalance` — ما يطالب به الناقل)
- **رصيد دفترنا** (`loadOpenBalance(carrierId)` قبل الاستيراد)
- **الفرق** (أخضر إن ≤ 0.5 ر.س، ذهبي إن أكبر)

دفاعي وموثوق: لا يعتمد على مطابقة سطور فردية. يكشف فوراً إن كان دفترنا
يطابق ما يطالب به الناقل قبل الحفظ.

### 5.2 لماذا لا تُطابَق سطور COD آلياً (تحذير)
> **لا تبنِ matcher آلي يربط «COD 586832» في كشف سمسا مع تحصيلاتنا المسجّلة.**
> هما **كائنان ماليان مختلفان**:
> - سطور COD في كشف سمسا (586832=19,560.88، 663732=17,176.21، 712060=12,874.66 ·
>   إجمالي ~49.6 ألف) = **مستندات فوترة COD صافية** (Amount Applied يُصفّر معظمها).
> - تحصيلاتنا المسجّلة (`cod_settlement` direction='in' لسمسا = **259,273.02** عبر
>   763 شحنة، 3 رفعات) = **النقد المُستلَم per-AWB**.
> - **لا يوجد مفتاح مشترك**: `cod_settlement.settlement_ref = null` لكل رفعات سمسا.
>
> أي مطابقة per-line ستكون مضلّلة. لتمكينها مستقبلاً: التقاط رقم دفعة سمسا
> (batch no) في `settlement_ref` عند رفع ملف التحصيل، ثم المطابقة عليه.
